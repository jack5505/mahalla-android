...
- **Приложение против живого стенда.** `/v3/api-docs` поднят, снят и разобран:
  **180 эндпоинтов** в 164 путях. Клиент зовёт **77 уникальных** (79
  ...
  на сегодня изжит.
- **Разница 103 ручки — не «отставание на порядок».** Больше половины это
  бизнес-панель, админка и кабинет мастера. Клиентских незакрытых областей
  шесть, и четыре из них уже в открытых PR.
...
- **Техдолг перепроверен по коду, а не перенесён списком.** Закрылось само:
  ...
  ушёл. Осталось: ноль `<plurals>` при 760 строках, `BottomNavItem.route: Any`,
  мёртвый `SearchEvent.QueryCleared`, `synchronized` + `runBlocking` в
  `TokenAuthenticator`.
...
- **`.claude/rules/testing.md` называет 163 тест-класса**, их 177. Правку
  `.claude/**` окружение агента не пропускает — число записано в
...
SUCCESSFUL. Прогон до правок и после: менялись только `.md`.
