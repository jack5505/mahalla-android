...
- **Единица суммы платежа не выводится.** У `PaymentTransaction.amount` нет
  дробного близнеца `amountSom` — в отличие от кошелька (issue #62) и цен
  тарифа, вывести делитель нечем. Читается как тийины: так названы денежные
  поля в остальной схеме (`mrrTiyin`, `walletRevenueTiyin`), и тот же делитель
  приложение уже использует, отправляя пополнение. Если бэкенд хранит платежи
  в сумах, суммы в истории будут в сто раз меньше — **это первое, что надо
  проверить живым ответом**.
