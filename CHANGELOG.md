...
## Этап: таб «Заказы» → «Мои активности» (issue #73, задача T7)

`feature/orders/ui/OrdersScreen.kt` был 16 строками `ScreenSkeleton` без
единого запроса: один из четырёх табов нижней навигации не делал ничего.
Теперь это один список из всех вертикалей — новая фича `feature/activity/`,
старый файл удалён.

**Спецификация T7 живёт в `docs/TASKS-BACKLOG.md` на невлитой ветке
`claude/issue-59-20260830-0152`** — в `main` этого файла нет, и ссылка из issue
битая. Текст снят через `gh api` с той ветки (та же грабля, что была с T8).
...
| билеты в кино | `GET cinema/tickets/my?page&size` | `PageResponseCinemaTicket` |

Все пять требуют Bearer (`401 UNAUTHORIZED` анонимно) — значит `ActivityApi`
собирается на **основном** Retrofit, а не на «голом» `@RefreshClient`.
Пагинация у всех настоящая и одинаковая (`page`/`size` → `content`,
`page`, `totalPages`, `last`).

**Заказы читаются общей ручкой `GET orders`, а не `food/orders/my` из текста
задачи.** Причина та же, по которой в issue #9 экран статуса читает
`GET orders/{orderId}`: у общей ручки ответ описан схемой `OrderView`
однозначно и со всеми суммами, а у `food/orders/my` и `fashion/orders/my`
имя схемы `OrderResponse` в `/v3/api-docs` перекрыто коллизией springdoc —
под ним лежит заказ **фрилансера** (`freelancerId`, `serviceTitle`), то есть
имена полей оттуда взять нельзя. Заодно один запрос вместо трёх: без параметра
`vertical` приезжают заказы всех вертикалей сразу, включая `CLOTHING` и
`PHARMACY`, которых в списке источников T7 не было вовсе.

- **Домен** `feature/activity/domain/`:
  - `ActivitySource` (пять ручек) — нужен не для красоты: по нему экран
    отмечает **сбойный раздел** при частичном отказе, а `Activity.key`
    (`source:id`) даёт уникальность в `LazyColumn` (у двух разных ручек id
    могут совпасть).
  - `ActivityKind` — что это: заказ еды/одежды/аптеки/кино/игровой зоны,
    бронь, запись к мастеру, запись к врачу, билет. Незнакомая вертикаль —
    `OtherOrder`, а не пропуск: заказ есть, деньги списаны, спрятать его хуже,
    чем назвать общим словом.
  - `ActivityStatus` — **общий вид четырёх разных перечислений** бэкенда
    (`NEW…REFUNDED` у заказов, `CONFIRMED…CANCELLED` у броней,
    `PENDING…NO_SHOW` у записей, `ACTIVE…REFUNDED` у билетов) с per-source
    разбором (`ofOrder`/`ofBooking`/`ofAppointment`/`ofTicket`). `isActive`
    и есть та граница, по которой делятся вкладки. `Unknown` считается
    **активным**: новое значение бэкенда не должно ни ронять экран, ни прятать
    живую активность в историю (то же правило, что у `OrderStatus.Unknown`).
    `ACTIVE` у билета — «действителен», а не «идёт сейчас», поэтому у него
    `Confirmed`, а у брони `InProgress`.
  - `ActivityMerge.filter` — отбор и порядок. Порядок у вкладок **разный**:
    «активные» по возрастанию («что дальше» — бронь на послезавтра не должна
    стоять выше заказа, который готовят сейчас), «история» по убыванию.
    Записи без даты идут в конец в обоих случаях, а при равном времени порядок
    задаёт `key` — иначе строки переставлялись бы при каждой перезагрузке.
  - `ActivityFeed` — ответ **всех** источников: `items`, `failures` по
    источникам, `nextPages` (курсор) и `requested`. Возвращается он, а не
    `ApiResult`, и это главное решение задачи (см. ниже).
- **Частичный отказ (требование T7)**: репозиторий не сводит пять независимых
  ручек к одному «получилось / не получилось». Четыре источника с данными и
  один с ошибкой — это список плюс строка «Не удалось загрузить: билеты в
  кино» **над** ним, названная именем раздела (иначе человек решит, что
  броней у него просто нет). В `ScreenState.Error` экран уходит только когда
  не ответил **ни один** источник (`ActivityFeed.isTotalFailure`) — так
  выглядит истёкшая сессия (401 у всех пяти) и отсутствие сети, и «вы ещё
  ничего не заказывали» там было бы враньём. `requested` в фиде нужен ровно
  для этого: при догрузке спрашивают уже не всех, и «отказали все» означает
  «все двое», а не «все пять».
- **Источники опрашиваются параллельно** (`async` в `coroutineScope`):
  последовательно это были бы пять сетевых задержек подряд, а зависят они друг
  от друга никак. Отказ одного остальных не отменяет — `apiCall` исключений не
  выпускает, поэтому `coroutineScope` не рушится.
- **Пагинация — курсор по источникам** (`Map<ActivitySource, Int>`), а не один
  номер страницы: страницы у пяти ручек кончаются в разное время, и просить у
  исчерпанного источника следующую значит получать один и тот же хвост заново.
  Источник, **не ответивший** на догрузку, из курсора не выбрасывается, а
  остаётся на своей странице — иначе одна неудачная страница навсегда выкинула
  бы весь раздел из догрузки. Поэтому и любой отказ догрузки, а не только
  полный, показывается хвостом списка: молча не догрузить часть списка — то
  же, что потерять её.
- **Фильтр «активные / история» сети не касается**: обе вкладки собираются из
  одной загрузки. Так и должно быть — «активное» это набор статусов, а
  `GET orders` принимает в параметре `status` ровно один, то есть на сервере
  это всё равно не отфильтровать. `vertical` и `status` в запрос не уходят.
- **Автотриггер догрузки висит на курсоре `nextPages`, а не на числе строк.**
  Вкладку отбирает клиент, и догруженная страница может целиком уехать в
  «историю»: число строк «активных» не изменится, `LaunchedEffect(itemCount)`
  не сработал бы ни разу, и крутилка в хвосте осталась бы висеть навсегда при
  `hasMore = true`. Курсор же сдвигается после каждой удачной страницы, а
  когда он пуст — хвоста в списке уже нет.
- **Три пустых состояния, а не одно**: «вообще ничего» — с кнопкой «посмотреть,
  что рядом» (требование T7: пустое состояние с действием, а не голое «ничего
  не найдено»; кнопка идёт через `navigateToTab`, потому что это переключение
  таба, а не переход вглубь); «нет активных, всё в истории» и «история пуста» —
  свои тексты. Предлагать «сходите на главную» тому, у кого двадцать заказов в
  истории, значит не заметить его самого.
- **Переходы — только заказы «Еды»** (`ActivityTarget`, правило то же, что у
  `NotificationTarget` в issue #81). `OrderStatusScreen` построен на домене
  «Еды»: он рисует этапы кухни и умеет «повторить заказ», возвращая позиции в
  **корзину еды**, — заказ одежды открылся бы там под видом заказа еды. У
  брони, записи и билета своих экранов ещё нет, и такие строки **не
  кликабельны**: нажатие без последствий читается как сломанный экран.
- **Названия заведения нет ни в одном из пяти ответов** (только `placeId`),
  поэтому заголовок строки — вид активности, а рядом идёт то, что бэкенд даёт
  словами: номер заказа, название услуги, место в зале. Показывать UUID или
  выдумывать имя нельзя.
- **Карточка — `OrderCard` из кита эпика 2 для всех пяти источников.**
  `BookingCard` и `TicketCard`, которые называет T7, не подошли:
  `BookingCardUi` требует числа гостей, `TicketCardUi` — номера талона и
  «сколько человек впереди»; ни одного из этих полей контракт не отдаёт — они
  про бронь столика и про очередь, которых у бэкенда пока нет. У `OrderCard`
  появился необязательный `onClick` (как у двух его соседей) — пустая лямбда
  оставляла бы рябь нажатия и роль кнопки для TalkBack там, где нажатие ничего
  не делает.
- **Дата записи собирается из `apptDate` + `startTime`**, причём тип
  `startTime` — `JsonElement` (правка issue #141 поверх первой версии, где
  стоял объект `{hour, minute, second, nano}`): форма `LocalTime` из схемы
  стенда не следует — springdoc описывает его объектом, а Jackson с
  `JavaTimeModule` отдаёт строку `"09:30:00"`. Жёсткий тип уронил бы разбор
  **всей страницы**, то есть из списка разом выпали бы оба источника записей.
  Разбирает значение `parseServerLocalTime` (`core/format/ServerLocalTime.kt`).
  Разворачивается местная дата в `Asia/Tashkent`, а не в UTC — иначе запись на 09:00
  показывалась бы как 14:00, а запись после 19:00 уезжала бы на следующий
  день. Время вне суток отбрасывается целиком: собранное из мусора время хуже
  отсутствующего, человек поверит цифрам на экране.
- **Время сортировки — время события**, если источник его даёт: начало брони,
  дата записи. У заказа и билета его нет, там `createdAt`. Разбор общий —
  `parseServerInstant` (Jackson отдаёт `LocalDateTime` без зоны).
- Разбор мягкий: запись **без id отбрасывается** (дубликат ключа в
  `LazyColumn`), всё остальное необязательно — активность без статуса, суммы и
  даты в списке остаётся. За ней стоят потраченные деньги, и «заказ исчез»
  страшнее «заказ без даты».
- Строки `orders_title`/`orders_subtitle` удалены (обеих локалей) вместе с
  заглушкой; подпись таба в нижней навигации — прежняя `nav_orders`.
  `navigateToTab` в `MahallaApp.kt` стал `internal`.

Проверено: `./gradlew testDebugUnitTest` — **1142 теста в 118 классах, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
`assembleRelease` — BUILD SUCCESSFUL. Новые тесты: `ActivityDomainTest`
(разбор четырёх наборов статусов, регистр и разделители, незнакомый статус
остаётся активным, вид заказа по вертикали, ключ строки с источником, порядок
обеих вкладок, записи без даты в конце, стабильность при равном времени,
дедупликация склейки, полный и частичный отказ, «отказали все» при догрузке),
`ActivityRepositoryTest` (MockWebServer с `Dispatcher` по пути — пять
запросов идут параллельно, и `enqueue` раздал бы ответы вразнобой: пути и
query, конверт, слияние пяти источников, кликабельна только «Еда», время
брони по началу и фоллбэк на создание, дата записи с `LocalTime` в
Ташкенте, битое время не выдумывает момент, запись без id отброшена,
активность без полей осталась, один сбойный источник и 401 у всех, 2xx с
`success:false`, три правила `hasMore`, догрузка спрашивает только заданные
источники), `ActivityViewModelTest` (слияние, частичный отказ не прячет
список, полный отказ — единственная ошибка экрана, пустой ответ не ошибка,
фильтр без обращения к сети, пустая вкладка ≠ пустой экран, догрузка и
дедупликация, сбойный источник остаётся в курсоре, частичный отказ догрузки
виден в хвосте, «повторить» не гасит список, перечит на возврате,
pull-to-refresh, переход по заказу еды, молчание на строке без экрана и на
исчезнувшей строке); фейк `FakeActivityRepository`; новый случай в
`GraphAssemblyTest`.

**Не сделано / риски:**

- **Ни один успешный путь не проверен на живых данных**: все пять ручек
  требуют Bearer, а SMS-кода в CI нет — анонимно проверено только то, что
  эндпоинты существуют и отвечают `401 UNAUTHORIZED`. **Первое, что надо
  посмотреть руками под токеном: что `GET orders` действительно отдаёт заказы
  всех вертикалей** (в схеме `OrderView.vertical` включает все пять, но это
  тот же риск, что записан в issue #9 про `GET orders/{orderId}`).
- **На устройстве не проверено** (эмулятора в CI нет): список, фильтр,
  догрузка, пустые состояния.
- **Единица цены не подтверждена** — тот же риск, что в issue #9: суммы
  приходят целыми числами без дробного близнеца (в кошельке пара
  `balance`/`balanceSom` есть, здесь нет). Считаем сумами. Окажутся тийины —
  суммы будут в сто раз больше.
- **Заголовки строк безымянные**: пока бэкенд не отдаёт название заведения ни
  в одном из пяти ответов, в списке будет «Заказ еды», а не «Osh Markazi».
  Задача для `jack5505/mahalla` — `placeName` в `OrderView`, `GamingBooking`,
  `AppointmentResponse` и `CinemaTicket`; она же закрыла бы имя заведения на
  экране статуса заказа (issue #9).
- **Время сеанса у билета в кино неизвестно** (в `CinemaTicket` только
  `sessionId`), поэтому билет сортируется по времени покупки — единственный
  источник, где так. Туда же: `qrCode` приходит, но показать его негде —
  экрана билета нет.
- **Четыре из пяти источников никуда не ведут**: брони, записи и билеты
  станут кликабельными вместе со своими экранами (эпики #10, #11, #14).
  `durationHours` брони и `endTime` записи по той же причине объявлены в DTO,
  но в домен не доезжают.
- **Заказы фрилансера (`GET freelancers/orders/my`) в список не включены**:
  их схема — та самая коллизия `OrderResponse`, из которой имена полей и
  берутся вслепую. Появятся вместе с вертикалью мастеров.
- **«Мои заказы» и «мои активности» — одно и то же место**: отдельного входа
  из карточки заведения или из истории кошелька нет.
- **Автодогрузка идёт цепочкой, пока хвост списка на экране**: у человека с
  парой активностей это значит вычитать все страницы всех источников за
  несколько запросов. Ограничитель тут один — то, что хвост уходит из
  композиции, стоит прокрутить список. Если у кого-то история окажется
  большой, это первое, что стоит заменить на кнопку «показать ещё».
- Скриншот-тестов по-прежнему нет: обе вкладки, отметка сбойного раздела и три
  пустых состояния проверялись глазами по `@ThemeLanguagePreviews`.

**Хвост по замечаниям ревью (issue #147).** Итог этапа выше изначально ушёл в
`AGENTS.md` — файл инструкций, который грузится в контекст каждой сессии;
перенесён сюда, как и договаривались 2026-09-06. Заодно в
`docs/API-CONTRACT.md` появился раздел `ActivityApi` (пять ручек и то, чем
`GET orders` **без** `vertical` отличается от вызова с ним из «Одежды»), а
разделитель заголовка строки уехал из кода в строковый ресурс
`activity_title_with_note` — в остальных экранах склейка идёт так же.
Цифры прогона в абзаце «Проверено» — с ветки эпика, до слияния с `main`.
