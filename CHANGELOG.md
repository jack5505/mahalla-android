...
## Этап: «не авторизован» посреди приложения (issue #138, ветка claude/issue-138-*)
...
- **`SessionExpiry`** (`data/prefs`) — одно событие «сессия кончилась не по
...
- **`MahallaApp`** на событие уходит на `WelcomeRoute` и показывает
  ...
  из уведомления мимо основного графа. Внутри графа онбординга, на вводе
  адреса сервера и на экране обновления событие игнорируется: там сессии и не
  было, а welcome посреди ввода кода стёр бы шаг. Анкеты последнего шага
  регистрации лежат **вне** графа онбординга (открываются ещё и из профиля) —
  их исключение не покрывает, и незаполненная анкета потеряется: отправить её
  всё равно нечем. Снекбар-хост появился в корневом `Scaffold` — до этого
...
- **Заодно: refresh больше не убивает сессию при обрыве связи.** Было
  ...
  `apiCall`, и сессию заканчивает только отказ: 401/403, `success: false`,
  `400`/`422` или 2xx без токенов. Обрыв, таймаут, 5xx, 404, битое тело и
  отдельно **429** сессию не трогают — запрос просто проваливается, как любой
  другой. Без этого новый выход на экран входа срабатывал бы от секундной
  потери сети или от троттлинга, а стоит он платного SMS и регистрации
  заново.

Проверено: `./gradlew testDebugUnitTest` — **1855 тестов в 178 классах, 0
падений**; `assembleDebug` — BUILD SUCCESSFUL; `lintDebug` — BUILD SUCCESSFUL.
Новые тесты: `SessionExpiryEffectTest` (compose-тест под Robolectric на
настоящем `NavHost`: экран основного графа и деталь вне него уходят на вход с
пустым стеком, онбординг/адрес бэкенда/обновление не трогаются, второе событие
не кладёт второй welcome), `NetworkStackTest` (провал refresh стирает сессию
**и** сообщает об этом, `success:false` и 200 без токенов — то же, обрыв связи,
503 и 429 сессию сохраняют и молчат, удачный refresh молчит, 401 без сессии
молчит), `RootViewModelTest` (пройденный онбординг без сессии всё равно
начинается с входа, с сессией — с основного графа, корень отдаёт событие, а не
пустой flow). **Каждый новый тест проверен обратным прогоном**: с убранной
правкой краснеет — в том числе при подмене `popUpTo(graph.id)` на
`popUpTo(MainGraph)` (падает деталь вне графа) и при снятом исключении для
онбординга.

**Не сделано / риски:**

- **Причина 401 именно у этого пользователя не установлена.** Логов нет, на
  скриншоте только текст сервера, а `SENTRY_DSN` в репозитории до сих пор не
  задан — падения и отказы у пользователей невидимы. Закрыта та дыра, которую
  видно по коду: мёртвая сессия больше не оставляет человека внутри
  приложения.
- **Один путь 401 остался необработанным намеренно**: refresh **удался**, а
  ...
  вход в этом случае и не помог бы. Оформлено issue.
- **`Serialization` на refresh сессию больше не заканчивает** (раньше
  `runCatching{}.getOrNull()` стирал её и здесь). Ставка на то, что 200 с
  неразбираемым телом — это чаще подменённый ответ публичного Wi-Fi, чем
  отказ бэкенда. Если же так сломается контракт (а он ломался четырежды за
  месяц), приложение останется в состоянии «везде 401» до ручного выхода в
  профиле. Оформлено issue.
...
- **Экраны под токеном по-прежнему показывают «повторить» на 401** — доля
  секунды до перехода на вход. Отдельного состояния «нужен вход» у них нет, и
  заводить его ради этого мгновения не стал.
