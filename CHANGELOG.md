...
## Этап: «Мои активности» — таб «Заказы» (issue #73) + типизация `startTime` (issue #141)

Вертикаль писалась в ветке PR #110 и лежала там непринятой, пока `main` ушёл
вперёд на 70 коммитов. В `main` она приехала слиянием с разрешением конфликтов
в пяти файлах (`AGENTS.md` — версия `main`, `MahallaNavHost.kt` — импорты обеих
сторон, `values/`+`values-ru/strings.xml` — строки обеих сторон,
`GraphAssemblyTest.kt` — все три теста).

- **`feature/activity/`**: один список из пяти источников (`orders/my`,
  `gaming/bookings/my`, `appointments/my`, `hospitals/appointments/my`,
  `cinema/tickets/my`), вкладки «Активные»/«История» на клиенте, догрузка
  постранично у каждого источника со своим курсором, дедупликация по
  `source:id`. Экран `orders/ui/OrdersScreen.kt` (заглушка) удалён вместе со
  строками `orders_title`/`orders_subtitle`.
- **Частичный отказ не роняет список** (требование T7): сбойный источник —
  строка над списком, `ScreenState.Error` только когда не ответил **никто**.
- **`startTime`/`endTime` записи — `JsonElement?`, не объект** (issue #141).
  Грабли: springdoc описывает `java.time.LocalTime` объектом
  `{hour, minute, second, nano}`, Jackson с `JavaTimeModule` отдаёт строку
  `"09:30:00"`. Жёсткий тип уронил бы `SerializationException` на разборе
  **страницы**, то есть из списка выпали бы разом оба источника записей —
  и мастер, и врач. Разбор — общий `core/format/ServerLocalTime.kt`, тот же,
  что у брони; `LocalTimeDto` и приватный `toLocalTime` из
  `feature/activity/data/` удалены. Форма поля дописана в
  `docs/API-CONTRACT.md`.

### Что нашло ревью диффа перед мержем и что исправлено

- **Пустая вкладка гасила догрузку.** `EmptyTab` рисовался до хвоста списка, а
  автотриггер догрузки стоит именно в хвосте: у человека с двадцатью
  завершёнными записями на первой странице и активной на второй вкладка
  «Активные» говорила «активных нет» и больше ничего не запрашивала. Теперь
  решение принимает `ActivityState.showsEmptyTab` — пустая вкладка только когда
  курсор догрузки пуст.
- **Полный отказ давал шесть красных блоков.** `sourceFailures` заполнялись всеми
  пятью источниками одновременно с `ScreenState.Error`, и на истёкшей сессии
  экран показывал пять отметок разделов плюс блок общей ошибки — шесть кнопок
  «Повторить». При полном отказе отметки разделов больше не выставляются.

### Что осталось

- **Дубли контракта.** Все пять ручек и их схемы уже объявлены в
  `feature/booking`, `feature/hospital`, `feature/gaming`, `feature/cinema` —
  DTO «активностей» стали вторым экземпляром тех же схем. Именно это
  расхождение и породило #141: в booking-версии тип был правильным с самого
  начала. Просится в отдельную задачу.
- **Подпись таба — по-прежнему `nav_orders`** («Заказы»), а заголовок экрана —
  «Мои активности». Дизайн-репо в этом прогоне недоступно, менять подпись без
  ТЗ не стал.
- **Гонки загрузок.** `load()` не гасит предыдущий свой job: «Повторить» при
  показанном списке не блокирует следующий `ScreenResumed`, и два `load()`
  могут идти параллельно, а pull-to-refresh поверх такого повтора погасит
  спиннер по первому ответу. Указало ревью, на экране не воспроизводилось.
- **Три ручки в `docs/API-CONTRACT.md` помечены «схема не проверена»** — все
  под токеном, а `CONTRACT_REFRESH_TOKEN` в CI нет. Вертикаль читает их поля,
