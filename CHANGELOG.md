...
вовсе. Расхождение записано здесь же с issue #42 и #51 как осознанное, но
незакрытое: «при появлении app-lock его придётся закрывать». Закрыто.
...
| `PUT pin/biometric` | `{enabled, deviceId, pin}` — **PIN обязателен** → `ApiResponseBoolean` | `401` |
| `POST pin/verify` | `{pin, deviceId, purpose?}` → `{success, actionToken, remainingAttempts, lockedSecondsRemaining, message}` | `401` |
| `POST pin/set`, `POST pin/reset` | требуют пары `otpToken` + `otpCode` | `401` |
| `DELETE pin` | query `deviceId` | `401` |
...
**Приятная новость про схему**: коллизии springdoc здесь нет — `PinStatusResponse`,
`ChangePinRequest`, `BiometricToggleRequest`, `CheckSessionRequest`,
`PinResumeRequest` встречаются по одному разу, и поля взяты как есть. Это
первая задача за несколько, где тело запроса не пришлось выводить из ответа.
...
- **`auth/session/check` и `auth/pin-resume` требуют Bearer** — в отличие от
  ...
  собираются на **основном** Retrofit, а не на «голом» `@RefreshClient`, где
  живёт остальная авторизация. Это же и логично: `pin-resume` — не вход, а
  продолжение существующей сессии, и чужой аккаунт (issue #86) через него
  приехать не может.
- **Гео-заголовки обязательны у `pin/…`**: без `X-Geo-Lat`/`X-Geo-Lng`
  ...
  `GeoHeaderInterceptor` (issue #53), так что вопрос закрылся сам собой.

### Пункт 4 issue: источник истины у PIN

**Сервер — источник истины, локальный Keystore-хэш это его офлайновая копия
того же кода.** Инвариант, который держит `SecurityRepository`: локальный хэш
пишется **только** PIN'ом, который бэкенд уже принял — `auth/setup-pin`,
`auth/pin-login` (оба с issue #51), `pin/change` и `auth/pin-resume`. Двух
разных PIN в приложении больше нет.

Почему не «только сервер, локального нет»: экран блокировки, которому нужна
сеть, превращает приложение в кирпич в метро и в самолёте — ровно тот риск, о
котором предупреждает issue («ошибка здесь запирает пользователя вне
приложения»). Поэтому app-lock проверяет код локально, а `pin-resume`
вызывается **после** успешной проверки и его отказ разблокировку не отменяет:
он продлевает серверную сессию, а не решает, пускать ли человека в его же
приложение.

Отсюда же обработка редкого отказа Keystore при смене PIN: сервер новый код
уже принял, а записать его локально не вышло. Прежний код в хранилище
оставить нельзя (он открывал бы замок кодом, которого бэкенд не знает), врать
про неудачную смену тоже нельзя. Поэтому локальная копия **стирается**, а
замок без неё просто не вооружается и включится обратно при следующем входе
или разблокировке. Состояние замка из-за этого показано на экране
безопасности отдельной плашкой — молчаливое «замок выключился» человек должен
узнать не от вора.

### App-lock

- **`AppLockManager`** (`feature/security/domain`) — синглтон с
  `StateFlow<Boolean> locked`. Вооружается, только когда **есть сессия и есть
  локальная копия PIN**: замок, который открыть нечем, и есть запирание
  пользователя вне приложения. Отказ хранилища читается как «не вооружён» по
  той же причине.
- **Отсрочка 30 секунд** (`GRACE`) обязательна: код из SMS копируют в
  приложении сообщений, вход через Telegram уходит в Telegram (issue #46),
  пополнение кошелька — в браузер провайдера (issue #93). PIN на возврате из
  этих трёх сценариев — не безопасность, а способ не дать человеку закончить
  дело. Часы, переведённые назад, дают отрицательный интервал — это «измерить
  не удалось», и тогда запираем.
- **`ProcessLifecycleOwner`** (`AppLockObserver`, новая зависимость
  ...
  отличает настоящий уход в фон от пересоздания Activity, а приложение
  пересоздаёт её на смене языка (эпик 1.5) и темы. Своя реализация запирала бы
  человека каждый раз, когда он переключает язык в профиле. Подписка встаёт в
  `MahallaApplication.onCreate` — второе исключение из правила «в Application
  ничего не появляется», после отчётов о падениях.
- **Экран блокировки — оверлей поверх всего** (`MainActivity`), а не маршрут:
  замок обязан накрывать любой экран, включая онбординг с его собственным
  стартовым пунктом, экран обновления (issue #80) и ввод адреса бэкенда (issue
  #26). Маршрутом он вмешивался бы в back stack и в deep links; оверлей строго
  добавочный.
- **`init` у `AppLockViewModel` нет**: она привязана к Activity и переживает
  ...
  `AppLockEvent.Shown` на каждом появлении экрана. Грабля неочевидная, тест её
  закрепляет.
- **«Назад» сворачивает приложение** (`moveTaskToBack`), а не открывает замок и
  не глотается молча: под оверлеем живой экран с деньгами и заказами, а
  проглоченное нажатие читается как сломанная кнопка.
- **Мёртвую сессию ловит `session/check`**: `sessionValid: false` → выход и
  вход заново. Ждать PIN от того, у кого сессии нет, значит запереть его
  насовсем. Отказ сети сюда **не** доезжает намеренно: «спросить не удалось» —
  не «сессии нет», а мёртвый токен приложение всё равно узнает по первому 401.
- **Лимит попыток на замке — локальный, и это не противоречит issue.** Свой
  счётчик запрещён в *серверном* режиме (`pin-login`, `pin/change`,
  `pin/biometric` — там считает бэкенд), а экран блокировки проверяет код без
  сети, и считать попытки серверу нечем. Исход — не «заблокировано навсегда», а
  выход в SMS-вход: те же пять попыток и то же поведение, что у `PinViewModel`
  с эпика 3.4.
- **Биометрия открывает замок локально**: ручки «продолжить сессию по
  отпечатку» у бэкенда нет, есть только `pin-resume` с PIN. Так что после
  отпечатка серверу не сообщается ничего — сессия продлится сама на следующем
  успешном запросе.

### Смена PIN и переключатель биометрии

- **Экран «Безопасность»** (`SecurityRoute`, строка в профиле — всем): статус
  PIN с бэкенда, состояние замка, смена PIN и переключатель биометрии.
  Отдельный экран, а не строки в профиле: у PIN и биометрии одно общее правило
  («подтверди кодом»), и разложенные по списку настроек они читались бы как
  несвязанные переключатели.
- **Смена PIN** (`ChangePinRoute`): текущий → новый → повтор, каркас общий с
  шагами онбординга. Новый код, равный текущему, отсекается **до** сети: «сменил
  PIN на тот же самый» — не смена, а человек уверен, что защитился. Несовпавший
  повтор переспрашивает только новый код — текущий уже ввели верно. Отказ
  сервера показывается его текстом (issue #34) и возвращает к первому шагу;
  успех **не уводит с экрана сам** (issue #49), а показывает подтверждение.
- **Переключатель биометрии требует PIN** — так решил бэкенд
  (`BiometricToggleRequest.pin` обязателен). Порядок при включении: **датчик →
  код → сервер**. Обратный порядок записал бы «биометрия включена» до того, как
  выяснилось, что датчик не узнаёт хозяина. Выключение датчика не требует:
  человек как раз говорит, что пользоваться им не будет.
...
- **Молчание сервера о новом значении — не отказ**: `ensureSuccess()` уже
  подтвердил переключение, и состояние выводится из того, о чём просили.
- **Три ручки из семи не подключены, и это осознанно**: `pin/set` и `pin/reset`
  требуют свежего SMS-кода (установку PIN приложение делает через
  `auth/setup-pin` на входе, а «сброс по SMS» у него — это выход и вход заново,
  путь короче и без второго кода); `DELETE pin` удалил бы PIN при живой сессии,
  то есть молча выключил бы app-lock. `pin/verify` не понадобился: подтверждение
  кодом делают сами `pin/change` и `pin/biometric`.
...
Ровно то, о чём предупреждает раздел про `Color.kt` выше; `kspDebugKotlin`
падает с `Unclosed comment`.
...
Проверено: `./gradlew testDebugUnitTest` — **1403 теста в 138 классах, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
`assembleRelease` — BUILD SUCCESSFUL. Новые тесты: `ServerPinStatusTest`
(молчание сервера у обеих схем, отрицательный остаток блокировки, правила
формата кода), `AppLockManagerTest` (короткая отлучка и длинная, первый запуск,
часы назад, нет сессии, нет локальной копии, отказ хранилища, `unlock`,
`lockNow`, `disarm`, повторная оценка при уже закрытом замке),
`SecurityRepositoryTest` (MockWebServer: query и тела всех трёх ручек `pin`,
конверт, инвариант «локальный хэш пишется только принятым кодом», уборка после
отказа Keystore, негодный код и код «сам в себя» не доходят до сети, тело
`session/check` и `pin-resume`, ответ без токенов не роняет разблокировку,
2xx с `success:false`, фактический `401` стенда), `AppLockViewModelTest`
(разблокировка без сети, продолжение сессии, неверный код, отказ хранилища не
тратит попытку, исчерпанные попытки, биометрия, мёртвая и недоступная сессия,
«забыли PIN», второе запирание подряд), `ChangePinViewModelTest`,
`SecurityViewModelTest`; фейк `FakeSecurityRepository`, новые случаи в
`GraphAssemblyTest` и `RoutesSerializationTest`. `FakePinStorage` научился
отказывать только на записи (`saveFailure`), `OnboardingRepository` —
`clearCompleted()`.

**Не сделано / риски:**

- **На живом аккаунте не проверено** (эмулятора в CI нет, вход требует
  SMS-кода): успешный ответ ни одной из ручек под токеном никто не видел —
  ...
  новым кодом, а `PUT pin/biometric` — что переключатель доезжает до сервера.**
- **Критерий «app-lock срабатывает при возврате из фона» тестами закрыт, а
...
- **Отсрочка 30 секунд выбрана на глаз** по практике банковских приложений.
  ...
  — это решение продукта, а «никогда» ещё и кнопка «сделать мои деньги
  доступными любому, кто взял телефон».
- **`pinRequired` из `session/check` приложение не использует как команду**:
  замок защёлкивается по клиентскому правилу (фон + отсрочка), а ответ сервера
  читается только ради `sessionValid`. Если бэкенд начнёт требовать PIN по
  своему таймауту, это надо будет учесть — сейчас такой сценарий кончится
  обычным 401.
- **Что именно бэкенд считает «залоченной» сессией, из схемы не следует**:
  `pin-resume` под токеном никто не звал, и не проверено, отвечает ли он
  успехом, когда сессия не заблокирована. Если он в этом случае отказывает,
  человек увидит текст сервера, но замок всё равно откроется — так и задумано.
- **Локальный лимит попыток живёт в памяти**: перезапуск приложения его
  сбрасывает. Это давнее замечание с эпика 3.4, и перенос в DataStore тут
  сознательно не сделан — цена ошибки в персистентном счётчике та же, что у
  всей задачи: человек, запертый вне приложения.
- **Смена PIN не отзывает другие сессии**: `revokeAll` бэкенду не отправляется.
  Вопрос к продукту — обычно смена кода поводом для этого считается.
