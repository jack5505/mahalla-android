...
- **`language` — выбора нет.** Слово `language` встречается во всей схеме
  **один раз**, в `MeResponse`. Ни одно тело запроса его не принимает
  ...
  пробой, issue #242. В тела запросов язык не положить, значит на запись
  серверное значение недоступно
  на запись, читать его — значит молча сбрасывать выбор человека. Не
  разбираем. `AppLanguage` к тому же богаче: у него есть `SYSTEM`, у сервера
  только `UZ`/`RU`.
...
### Хвост: тарифы подписки тоже спрашивают права, а не анкету (issue #244)

Правило «оказывает услуги» осталось после #237 в одном экземпляре — в
профиле, — а `SubscriptionViewModel.audience()` продолжал выбирать набор
тарифов по локальной анкете. Расхождение было видно человеку: настоящий
`FOOD_OWNER`, анкету не заполнявший, в профиле видел «Мои заведения», а в
подписках — покупательские тарифы.

Правило вынесено в домен одной функцией `providesServices(formRole,
serverRole)` (`feature/role/domain/ProvidesServices.kt`); её зовут и
`ProfileState.showMyPlaces`, и новое `RoleProfile.providesServices`, которое
спрашивает подписка. `RoleProfile` получил поле `serverRole` — читать его
...
бизнес-тарифы. Правило от этого не страдает — оно и так шире анкеты, — но
после #243 станет строже само.
