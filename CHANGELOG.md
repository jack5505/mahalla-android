...
сортировка «активных» смешивает прошлое `createdAt` заказов с будущим временем
броней на одной оси; «повторить» и возврат на экран перезагружают с нулевой
страницы и теряют уже догруженный хвост вместе с позицией прокрутки; частичный
отказ при пустых остальных источниках показывает «вы ещё ничего не заказывали»;
`parseServerInstant` не принимает ISO-смещения кроме `Z` (это общий код, не
этого PR).
