...
Проверено: `./gradlew testDebugUnitTest` — **1889 тестов в 180 классах, 0
падений**; `lintDebug` — BUILD SUCCESSFUL; `assembleDebug` — BUILD SUCCESSFUL.
...
`FreelancerRepositoryTest` (поля услуг с живого ответа, `404` как «нет анкеты»,
тела `POST freelancers/me` и `POST`/`PUT me/services`, пустой `ApiResponseVoid`).
