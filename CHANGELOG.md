...
их дописывали. Ищи по номеру issue.
...
## Этап: загрузка изображений — `POST media/upload` (issue #101)

Загрузчика изображений в проекте не было вовсе: фото нельзя было ни показать,
ни отправить. Показ — задача #60 (Coil), **эта задача про вторую половину**:
выбор файла, сжатие, отправка, прогресс, отмена.
...
| эндпоинт | контракт | без токена |
|---|---|---|
| `POST media/upload` | `multipart/form-data`, часть **`file`**; query `entityType`, `entityId` — оба необязательны → `ApiResponseMediaFile` | `401`, а без гео-заголовков `403 GEO_PERMISSION_REQUIRED` |
| `GET media/entity/{entityId}` | `ApiResponseListMediaFile` | `401` |
| `DELETE media/{id}` | `ApiResponseVoid` | `401` |

`MediaFile`: `id, url, thumbnailUrl, type (IMAGE|VIDEO|DOCUMENT), fileSize,
originalName, entityId, entityType, ownerId, isPublic`. Приятная новость про
схему: имена `MediaFile` и `ApiResponseMediaFile` встречаются в `/v3/api-docs`
по одному разу — коллизии springdoc, из-за которой поля запросов приходилось
выводить из ответов (#76, #84, #97), здесь нет.

### Главное, что дал живой стенд: режет не бэкенд, а nginx

Ограничений (размер, типы) в схеме нет — issue просила снять их руками. Сняты
перебором:

```
1000 КБ тела → 401 UNAUTHORIZED (то есть запрос дошёл до приложения)
1024 КБ тела → 413 Request Entity Too Large, HTML от nginx
```

То есть `client_max_body_size` со значением по умолчанию — **1 МиБ**, и это
потолок для всей загрузки. Отсюда два следствия, определившие реализацию:

- **сжатие обязательно** — снимок с камеры на 12 Мп это 3–5 МБ, и без сжатия
  каждая попытка кончалась бы страницей nginx на английском;
- **размер проверяется до отправки** — `413` приходит от прокси, без конверта
  `{success,error}`, то есть текста для человека в нём нет вовсе (issue #34
  показала бы голый HTML). Тест это поведение закрепляет.

Лимит почти наверняка не задуман (для эндпоинта загрузки фотографий 1 МиБ —
это дефолт, который забыли поднять). Замечание бэкенду — в рисках ниже; когда
его поднимут, в приложении меняется одно число.

### Реализация (`feature/media/`)

- **Домен**: `MediaFile`/`MediaType`, `MediaUploadLimits` (бюджет файла =
  лимит nginx **минус** запас на обвязку multipart: границы частей и
  `Content-Disposition` — это не ноль, а промах здесь стоит `413` вместо
  загрузки), `MediaRejection` (три клиентских отказа) и `ImageScaling` —
  арифметика сжатия чистыми функциями. Вынесена из декодера намеренно: сам
  `BitmapFactory` на JVM не поднять, а ошибка именно в кратности прореживания
  видна только как «фото почему-то не грузится», а в угле поворота — как
  аватар боком.
- **`MediaApi`** — первый `@Multipart` в приложении (весь остальной API ходит
  JSON'ом). На **основном** Retrofit: ручка требует Bearer. Гео-заголовки
  ставит `GeoHeaderInterceptor` (issue #53), так что вопрос закрыт сам собой.
  `GET media/entity/{id}` и `DELETE media/{id}` **не объявлены**: показывать
  загруженное пока нечем (#60), а ручка, которую никто не зовёт, — это
  контракт, который никто не проверяет.
- **`AndroidImageCompressor`**: тип по `ContentResolver` до чтения байтов →
  границы (`inJustDecodeBounds`) → прореживание → поворот по EXIF → точный
  масштаб под 1600 px → качество JPEG сверху вниз (85…45), пока не влезет в
  бюджет. Не влезло ни на одном — отказ, а не отправка заведомого `413`.
  Всё на `Dispatchers.Default`: это счёт, а не ожидание. `OutOfMemoryError`
  ловится отдельно (`runCatchingCancellable` ловит только `Exception`) и
  уходит в отчёты (issue #74).
- **EXIF читается `androidx.exifinterface`**, а не `android.media`: на этом
  настаивает lint, и он прав — в платформенной реализации известные дыры
  разбора метаданных, а разбираем мы чужой файл. Это единственная новая
  зависимость (~60 КБ).
- **`ProgressRequestBody`** — байты пишутся кусками по 8 КБ с `flush` после
  каждого. Без `flush` прогресс врал бы: OkHttp сложил бы весь файл в свой
  буфер за один вызов и отчитался о 100 % раньше, чем ушёл первый пакет.
  `writeTo` может быть вызван не один раз (повтор после refresh'а токена) —
  тогда счёт начинается заново, и это правильнее: заново уходит весь файл.
- **`MediaRepository`**: сжатие → multipart → домен. Кэша нет и быть не может.
  **Отмена — обычная отмена корутины**: Retrofit обрывает вызов, OkHttp
  закрывает сокет, недописанное тело до сервера не доезжает; отдельного
  `cancel()` в интерфейсе нет. Ответ **без `url`** — отказ (`Serialization`):
  файл, адреса которого никто не знает, показать и сохранить нечем.
  Отклонённый на клиенте файл в сеть **не уходит вовсе**, а причина
  показывается своей строкой: сервер о такой попытке не знает, и
  `ApiError.Business` без текста приехал бы общим «Nimadir xato ketdi».
- **Photo picker** (`ActivityResultContracts.PickVisualMedia`) — **без единого
  разрешения**: `READ_MEDIA_IMAGES` со своей галереей просил бы доступ ко всем
  снимкам ради одного аватара. Наружу отдаётся строка, а не `Uri`: с ней
  работает и ViewModel, которую проверяют на чистом JVM.

### Точка применения: фото профиля (#61)

Из перечисленных в issue целей выбрана одна — аватар. У остальных не хватает
не загрузки, а того, к чему её привязать: `id` отзыва приложение не знает
(ответ `POST reviews` не разбирается, #76), а логотип заведения потребовал бы
значения `entityType`, словаря которых в схеме нет.

Экран профиля: «Выбрать фото» → сжатие → полоска прогресса с процентами и
«Отменить» → адрес сохраняется в `UserProfileStore`. Прогресс полоской, а не
крутилкой: на медленной связи она единственное, что отличает работу от
зависшего экрана. Отказы — текстом сервера с раскрывающимися подробностями
(issue #34), клиентские — своей строкой.

`entityId` уходит равным id пользователя, когда он известен (по нему
загруженное потом находится через `media/entity/{id}`), а `entityType` не
отправляется вовсе: выдуманное значение бэкенд запомнит, и разбирать это
придётся руками.

Проверено: `./gradlew testDebugUnitTest` — **1302 теста в 131 классе, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
`assembleRelease` — BUILD SUCCESSFUL. Новые тесты: `ImageScalingTest`
(прореживание, подгонка, панорама 8000×1, все восемь значений EXIF),
`MediaUploadTest` (бюджет меньше лимита nginx, типы, лестница качества, коды
отказов), `MediaRepositoryTest` (MockWebServer: `multipart/form-data` с
границей, часть `file` с именем и типом, query с `entityType`/`entityId` и без
них, прогресс 0→100 без повторов, отклонённый файл не доходит до сети, ответ
без `url`, конверт с `success:false`, `401`, **фактический `413` от nginx**,
отмена корутины), пять случаев в `ProfileViewModelTest` (адрес сохраняется, не
затирая имя и номер; прогресс; второй тап не заводит второй загрузки; отмена;
отказ не стирает прежнее фото), случай в `GraphAssemblyTest`; фейки
`FakeImageCompressor` и `FakeMediaRepository`.

**Не сделано / риски:**

- **Живой загрузки не было**: `media/upload` требует Bearer, а SMS-кода в CI
  нет — `401` приходит до разбора тела. Проверены анонимно только коды ответов
  и лимит nginx. **Первое, что надо проверить руками: файл доезжает и в
  ответе приходит `url`.**
- **Критерий готовности выполним только наполовину**: `MahallaAsyncImage` — из
  задачи #60, а её в `main` нет (Coil не подключён). Поэтому загруженное фото
  в приложении **не видно**: экран честно пишет, загружено оно или нет, а
  шапка профиля по-прежнему рисует инициалы. Появится Coil — в неё встанет уже
  сохранённый `avatarUrl`, без изменений в этой задаче.
- **Адрес аватара сохраняется только локально**, и это не выбор клиента: у
  бэкенда нет ни `PUT /users/me`, ни другой ручки, которой можно сообщить
  аватар (`UserInfo.avatarUrl` он отдаёт, но принимать не умеет — проверено по
  полной схеме). После следующего входа профиль перезапишется ответом сервера,
  и адрес пропадёт; сам файл при этом остаётся на сервере за `ownerId`.
  **Задача бэкенду** (`jack5505/mahalla`): либо `PUT /users/me` с `avatarUrl`,
  либо чтобы `media/upload` с известным `entityType` сам проставлял аватар.
- **Задача бэкенду про nginx**: поднять `client_max_body_size` для
  `/api/v1/media/**` (сейчас 1 МиБ) и отдавать отказ конвертом, а не HTML —
  иначе причину человеку показать нечем.
- **Словаря `entityType` в схеме нет** — пока приложение его не отправляет.
  Фото к отзыву (#76) и логотип заведения (#84) ждут именно этого значения
  плюс id сущности.
- **Ограничение по типам — клиентское**: что принимает сам бэкенд, из схемы не
  следует. Отклоняем всё, кроме JPEG/PNG/WebP/HEIC.
- **Удаления загруженного нет** (`DELETE media/{id}` не объявлен): экрана, где
  редактируется галерея, пока не существует.
- На устройстве ничего не проверено (эмулятора в CI нет): photo picker, поворот
  по EXIF, реальное время загрузки и отмена. Скриншот-тестов по-прежнему нет.

### Два слияния с `main` (PR #119)

Ветка дважды отставала от `main`, и оба раза это выглядело как «CI упал».

**Первый раз** (влилась вертикаль «Больницы», #117): **GitHub не смог собрать
merge-ref, поэтому workflow на `pull_request` не запускался вовсе** — у ветки не
было ни одного прогона, а PR показывал «проверки не пройдены». То есть «CI
упал» здесь означало «CI не смог начаться». Конфликты были текстовые
(`values/strings.xml`, `values-ru/strings.xml`, импорты в `GraphAssemblyTest`) и
разрешены в пользу обеих сторон.

**Второй раз** (2026-09-06, `main` ушёл на 28 коммитов вперёд: вертикали
«Аптека» #100, «Одежда» #108, «Мастера» #107, «Кино» #106, подписки #103, акции
#104, карта #126 и **разрез `AGENTS.md`** #131). Конфликт остался ровно один — в
`AGENTS.md`, и он структурный, а не смысловой: `main` сократил файл до правил и
унёс историю в `CHANGELOG.md`, а ветка дописывала раздел в старый длинный файл.
Разрешено по новой раскладке — `AGENTS.md` взят из `main` целиком, этот раздел
переехал в `CHANGELOG.md`, эндпоинты `media/**` дописаны в
`docs/API-CONTRACT.md`. Остальное (`strings.xml`, `ProfileScreen`,
`ProfileViewModel`, `GraphAssemblyTest`) слилось само: у «Медиа» и у вертикалей
общих файлов, кроме строк и графа, нет.
...
Раздел дописан при разрешении конфликта с `main` (PR #109): ветка правила
...
## Этап: вертикаль «Игровые зоны» (gaming) — зоны и бронирование (issue #98)

Категория `GAMING` в каталоге была с эпика 4, а забронировать в ней было
нечего: `gaming-controller` бэкенда приложение не знало вовсе, а
`PlaceAction.Booking` доезжал до эффекта `OpenVertical` и упирался в
`else -> Unit`.
...
| эндпоинт | контракт | без токена |
|---|---|---|
| `GET gaming/places/{placeId}/zones` | `data: [GamingZone]`; `placeId` — `uuid` | **`200`** с `data: []` — ручка анонимна |
| `POST gaming/bookings` | тело — см. ниже; ответ `GamingBooking` | `401 UNAUTHORIZED` |
| `GET gaming/bookings/my` | query `page`, `size` → `PageResponseGamingBooking` | `401` |
| `POST gaming/places/{placeId}/zones`, `PUT gaming/places/{placeId}/bookings/{id}/complete` | ведёт заведение (бизнес-панель, эпик #16) | `401` |

`GamingZone`: `id, placeId, name, description, zoneType, pricePerHour,
totalSeats, isAvailable`. `GamingBooking`: `id, zoneId, placeId, userId,
startTime, endTime, durationHours, totalPrice, status (CONFIRMED|ACTIVE|
COMPLETED|CANCELLED)`. Гео-заголовки обязательны и здесь (без них `403
GEO_PERMISSION_REQUIRED`), но их ставит `GeoHeaderInterceptor` (issue #53).

### Два расхождения с issue — оба на стороне бэкенда

1. **Тело `POST gaming/bookings` подтвердить нечем.** В схеме оно объявлено
   как `BookRequest`, а это имя перекрыто коллизией springdoc: на него
   ...
   тело записи к врачу. Живым запросом форму тоже не снять: `401` приходит
   **до** валидации (проверено и на пустом теле, и на заполненном). Поэтому
   поля названы так же, как в ответе того же эндпоинта — `{zoneId, startTime,
   durationHours}` — то же решение, что принято для отзывов (#76) и заявки
   продавца (#84).
2. **Отмены брони у бэкенда нет вовсе.** В `gaming-controller` ровно пять
   путей, `cancel` среди них нет; в общем `orders` для `GAMING` есть только
   `GET`. Требование issue «мои брони с отменой» закрыть нечем, поэтому кнопки
   отмены в приложении нет: она бы никуда не вела. Вместо неё экран говорит
   словами, что бронь снимают в самом заведении — молчание читалось бы как
   «отменить нельзя нигде».

### Что сделано

- **Домен** `feature/gaming/domain/Gaming.kt`: `GamingZone` (+ `isBookable` и
  `totalPrice`), `GamingBookingStatus` (+ `Unknown`), `GamingBooking`,
  `GamingBookingPage`, `GamingBookingDraft` + `GamingBookingValidator`,
  `GamingSlots`. Правила, которые стоит помнить:
  - **Цена `0` — не «бесплатно», а молчание сервера**: такую зону нельзя
    бронировать (`isBookable`) и её цена не показывается. Счёт неизвестного
    размера предлагать нельзя.
  - **`Unknown` активной броней не считается**: рисовать «предстоит» по
    незнакомому значению — придумывать за сервер. Молчание об `isAvailable`
    читается как «закрыта» (правило `MyPlace`, #94).
  - **Слоты считаются на клиенте**, потому что расписания зоны и занятых
    интервалов в контракте нет вовсе: получасовая сетка от ближайшего
    получаса **вверх** от «сейчас» (округление вниз дало бы слот в прошлом —
    грабля `DeliverySlots` эпика 5), 24 слота, сетка выравнивается по
    ташкентским часам. Занятое время видно только по отказу сервера.
  - Проверка черновика идёт **от одного «сейчас»** и при открытии шторки, и
    при отправке: слот, выбранный минуту назад, мог протухнуть.
- **Данные** `feature/gaming/data/`: `GamingApi` на **основном** Retrofit (две
  ручки из трёх требуют Bearer; отдельный клиент ради анонимных зон завёл бы
  второй пул соединений), `GamingRepository`, мягкий разбор. Кэша нет
  намеренно: зона из Room предложила бы бронь того, что уже занято (правило
  меню в #9). Разбор: зона **без `id`** отбрасывается (бронировать нечем, а в
  `LazyColumn` это дубликат ключа), всё остальное зону не прячет; `isAvailable`
  принимается и под именем `available` (Jackson сериализует `boolean` то так,
  то так — ошибка здесь увела бы в «закрыто» все зоны сразу, как `isRead` в
  #81). Незаполненный черновик в сеть не уходит
  (`ApiError.Business("GAMING_BOOKING_INVALID")`).
- **Ответ создания без `id` — не отказ**: бронь принята, а идентификатор
  приложению некуда девать (отменять нечем). Тот же приём, что у заявки
  продавца в #84. В списке «моих броней» запись без `id`, наоборот,
  отбрасывается — там это ключ.
- **Время начала уходит в UTC без зоны** (`2026-09-05T13:00:00`,
  `gamingRequestTime`). Это решение, а не мелочь: приложение читает зоне-менее
  время сервера как UTC (`parseServerInstant`), и стенд подтверждает
  договорённость своими метками (`"timestamp":"…Z"`). Отправив местное
  ташкентское, приложение получило бы ту же строку обратно, прочитало бы её
  ...
  `parseServerInstant`, вместе.
- **UI**: `feature/gaming/ui/zones/` — список зон (имя, тип, цена часа, места,
  бейдж «бронь недоступна»), бронь в шторке кита (слоты чипами, длительность
  степпером 1..8 часов, итог считается на глазах). Отказ сервера остаётся **в
  шторке** рядом с выбором (issue #34): закрыть её значило бы потерять и
  объяснение, и работу человека. Успех закрывает шторку и оставляет на экране
  плашку с зоной и временем — пустота читалась бы как «ничего не произошло».
  `feature/gaming/ui/bookings/` — «мои брони» страницами, с pull-to-refresh и
  перечитом на `ON_RESUME` (состояние брони меняет заведение, а не
  приложение).
- **Восемь часов — клиентский потолок брони** (`MAX_HOURS`): у бэкенда его
  нет, и он здесь не про правила сервера, а про лишний ноль — платит человек
  настоящими деньгами.
- **Два входа**: кнопка «Забронировать зону» на карточке клуба
  (`PlaceCapabilities.of(Playground) → gaming = true`) и строка «Мои брони» в
  профиле. Строка в профиле нужна: бронь берут с карточки клуба, и другого
  пути к своим броням, кроме как найти тот же клуб заново, у человека нет.
  `ordering` по-прежнему выключен («Заказать» — вне объёма #96 и #98).
- **`PlaceAction.Gaming` — своё действие, а не `Booking`** (решение при слиянии
  с `main`, см. ниже): пока ветка ждала, в `main` влилась вертикаль «Бронь»
  (#97), и `Booking` стал означать запись на время к мастеру
  (`barber-services` + `appointments`). Зона клуба — другой контроллер, другой
  список и другой экран, поэтому у неё своё действие, свой коллбэк
  (`onGamingClick`) и своя строка (`place_action_gaming`) — ровно как у
  `Doctor`, `Cinema`, `Shop` и `Products`, разведённых по той же причине.
- **Состояния разложены руками**, а не через `ScreenStateHost`: тот рисует
  `ApiErrorState` с собственной прокруткой, а внутри `LazyColumn` вложенная
  прокрутка меряется бесконечной высотой (#62).

Проверено: `./gradlew testDebugUnitTest` — **1268 тестов в 127 классах, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
`assembleRelease` — BUILD SUCCESSFUL. Новые тесты: `GamingTest` (разбор
статусов, правила зоны и итога, все причины черновика сразу, сетка слотов —
включая секунды и вырожденные параметры), `GamingRepositoryTest`
(MockWebServer: путь зон, оба имени флага доступности, зона без `id`,
пустой каталог стенда, тело и путь `POST` с временем без зоны, незаполненный
и протухший черновик не доходят до сети, бронь без `id`, незнакомый статус,
409 с текстом бэкенда, 2xx с `success:false`, query и конверт «моих броней»,
`last`/`totalPages`, отрицательная страница, фактический `401` стенда),
`GamingZonesViewModelTest` (14 случаев: загрузка, пустой ответ, повтор,
предвыбор слота, закрытая и незнакомая зона, итог и клампинг часов, отправка
выбранного, протухший слот, отказ сохраняет выбор, правка стирает отказ,
второй тап, перечит на возврате, путь в «мои брони», снятие плашки),
`GamingBookingsViewModelTest` (страницы, дедупликация, провал догрузки,
перечит, pull-to-refresh); новые случаи в `PlaceActionsTest`,
`RoutesSerializationTest` и `GraphAssemblyTest`; фейк `FakeGamingRepository`.

**Не сделано / риски:**

- **Успешный путь на живом стенде не проверен**: бронь и «мои брони» требуют
  Bearer, а SMS-кода в CI нет (`401` приходит до валидации тела). Проверено
  анонимно только чтение зон (`200`, `data: []`). **Первое, что надо проверить
  руками: тело `POST gaming/bookings` принимается бэкендом** — имена полей
  выведены из ответа, а не из схемы. Не совпадут — экран покажет
  `VALIDATION_ERROR` текстом сервера.
- **Задачи бэкенду** (`jack5505/mahalla`), без них issue закрыт не целиком:
  отмена брони (`POST gaming/bookings/{id}/cancel` или тот же путь в
  `orders`); развести коллизию springdoc у `BookRequest` — сейчас форма тела
  брони не документирована ничем; расписание зоны и занятые интервалы (без
  них слоты клиентские и «занято» видно только по отказу).
- **Каталог стенда пуст** (#53): заведения категории `GAMING`, с карточки
  которого открывают зоны, сейчас нет — проверять после наполнения каталога.
- **Единица `pricePerHour` и `totalPrice` не подтверждена**: поля целые, и
  дробного близнеца (`*Som`, как у кошелька в #62) у них нет. Считаем сумами,
  как в «Еде» (#9); если это тийины, все цены на экранах в сто раз больше.
- **Имя зоны в «моих бронях» пустое**: в ответе его нет ни в брони, ни в
  списке, а тянуть зоны каждого заведения ради подписи значило бы делать N
  запросов на экран. У только что созданной брони имя есть — его знает экран
  зон.
- **`GET orders?vertical=GAMING` не используется**: общий список активностей —
  это #73, отдельная задача.
- **Оплата брони не делается**: `totalPrice` приходит с сервера, но списания
  из кошелька в контракте брони нет — как и в «Еде» (#9), деньги остаются за
  заведением.
- На устройстве ничего не проверено (эмулятора в CI нет), скриншот-тестов
  по-прежнему нет: список зон, шторка и «мои брони» проверялись глазами по
  `@ThemeLanguagePreviews`.

### Слияние с `main` (PR #116, 2026-09-08)

Пока PR ждал, `main` ушёл на 228 файлов вперёд (вертикали #97/#99/#100/#103/
#104/#106/#107/#108, медиа #101, карта #126, разрез `AGENTS.md` #131, lint
обратно в CI). Самого гейминга в `main` нет, поэтому конфликтовали только общие
точки подключения — 13 файлов, из них 11 разрешились «взять обе стороны».

Смыслового конфликта было два:

- **`PlaceAction.Booking` теперь занят.** В `main` влилась вертикаль «Бронь»
  (#97), и `Booking` означает запись на время к мастеру; у мастеров
  `PlaceCapabilities.of` отдаёт `queue = true, booking = true`. Ветка же вешала
  на `Booking` игровую зону — после слияния кнопка клуба вела бы к услугам
  мастера, которых у клуба нет. Разрешено по правилу, которое `main` уже
  применил четыре раза (`Doctor`, `Cinema`, `Shop`, `Products`): у зоны своё
  действие `PlaceAction.Gaming`, свой флаг `PlaceCapabilities.gaming`, свой
  коллбэк `onGamingClick` и своя строка `place_action_gaming` (uz + ru).
  `PlaceActionsTest` это закрепляет: у клуба `Gaming`, и `Booking` в списке
  действий нет.
- **`AGENTS.md` разрезан** (#131): истории в нём больше нет. Файл взят из
  `main` целиком, этот раздел переехал в `CHANGELOG.md`, эндпоинты
  `gaming/**` дописаны в `docs/API-CONTRACT.md`.

Остальное — механическое сложение: строки и plurals обеих локалей, маршруты и
их тесты, `ProfileScreen` (строка «Мои брони» встала рядом с «Моими записями»),
импорты и случай в `GraphAssemblyTest`.

**Второй раз в тот же день** `main` ушёл вперёд на #109 (картинки, Coil) и #136
(харнесс сверки контракта `contract/*.sh` + пилот на «Брони»). Конфликтов
осталось три, все текстовые: `CHANGELOG.md` (обе стороны дописали раздел в одно
место — взяты оба, картинки выше гейминга по порядку слияния),
`values-ru/strings.xml` (`main` добавил пустую строку перед `</resources>`,
ветка — блок `gaming_*`) и `docs/API-CONTRACT.md` (соседние абзацы в разных
секциях). Раздел `GamingApi` при этом переписан под новую разметку статусов из
#136: у ручки зон ✅ (сверена анонимно), у двух под токеном ⚠️ с причиной
«нужен `CONTRACT_REFRESH_TOKEN`», и там же помечено, что `contract/gaming.sh` —
кандидат на пробу, когда токен появится. Смыслового конфликта не было:
`PlaceDetailsScreen` (галерея переехала на `MahallaAsyncImage`) и `ProfileScreen`
(секция аватара) слились сами — гейминг правил в них другие места.
