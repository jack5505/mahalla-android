...
- **`language` — выбора нет.** Слово `language` встречается во всей схеме
  **один раз**, в `MeResponse`. Ни одно тело запроса его не принимает
  (`UpdateMeRequest` — только `fullName` и `avatarUrl`), заголовка
  `Accept-Language` у API нет вовсе (единственный header-параметр во всём
  контракте — `X-Session-Id` у `auth/logout`). Серверное значение недоступно
