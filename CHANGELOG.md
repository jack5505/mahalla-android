...
сливаться. Здесь она перенесена на актуальный `main`; что при переносе
изменилось, отдельным списком ниже.

**Спецификация T7 живёт в `docs/TASKS-BACKLOG.md` на невлитой ветке
`claude/issue-59-20260830-0152`** — в `main` этого файла по-прежнему нет, и
ссылка из issue битая. Текст снят через `gh api` с той ветки.
...
- **Заголовки строк безымянные**: пока бэкенд не отдаёт название заведения ни
  ...
  экране статуса заказа (issue #9). Заведена: **issue #150**, там же время
  сеанса у билета и коллизия springdoc у `OrderResponse`.
