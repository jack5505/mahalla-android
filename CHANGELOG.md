...
Работа написана 4 сентября (PR #110), но за четыре дня `main` уехал — AGENTS.md
разрезали, добавились аптека, акции, подписки, `booking`, «мои записи», «мои
билеты», «мои заказы одежды» и харнесс сверки контракта, — и PR перестал
сливаться. 8 сентября её перенесли на тогдашний `main` (PR #152), и за сутки
повторилось то же самое: приехала вертикаль «Игровые зоны» (#98), #152 стал
`CONFLICTING`. Этот заход — **третий** перенос, на `main` от 9 сентября; что
менялось при переносах, отдельными списками ниже. **#110 и #152 можно
закрывать.**

**Спецификация T7 в `main` так и не появилась.** 4 и 8 сентября текст снимали
через `gh api` с невлитой ветки `claude/issue-59-20260830-0152`. К 9 сентября
`docs/TASKS-BACKLOG.md` переписан новой волной (`claude/issue-59-20260909-0656`,
PR #172): нумерация продолжена с T14, а от T7 там осталась строка таблицы
«написан **дважды**: PR #152 и #110». Полного текста задачи в актуальном
бэклоге больше нет — реализация сверялась со спецификацией прошлых заходов.

**Контракт снят со стенда** (`/v3/api-docs` + прямые curl'ы), пути — в
[docs/API-CONTRACT.md](docs/API-CONTRACT.md), раздел `ActivityApi`. Все пять
ручек требуют Bearer (`401 UNAUTHORIZED` анонимно) — значит `ActivityApi`
...
`PHARMACY`, которых в списке источников T7 не было вовсе. К тому же выводу
позже независимо пришла и одежда — `FashionApi.myOrders` ходит туда же.
...
- **Домен** `feature/activity/domain/`:
  ...
  - `ActivityFeed` — ответ **всех** источников: `items`, `failures` по
    источникам, `nextPages` (курсор) и `requested`.
- **Частичный отказ (требование T7)**: репозиторий не сводит пять независимых
  ручек к одному «получилось / не получилось» — поэтому `feed()` возвращает
  `ActivityFeed`, а не `ApiResult`. Четыре источника с данными и один с
  ошибкой — это список плюс строка «Не удалось загрузить: билеты в кино»
  **над** ним, названная именем раздела (иначе человек решит, что броней у
  него просто нет). В `ScreenState.Error` экран уходит только когда не ответил
  **ни один** источник (`ActivityFeed.isTotalFailure`) — так выглядит истёкшая
  сессия (401 у всех пяти) и отсутствие сети, и «вы ещё ничего не заказывали»
  там было бы враньём. `requested` в фиде нужен ровно для этого: при догрузке
  спрашивают уже не всех, и «отказали все» означает «все двое», а не «все
  пять».
...
- **Фильтр «активные / история» сети не касается**: обе вкладки собираются из
  ...
  это всё равно не отфильтровать.
...
- **Три пустых состояния, а не одно**: «вообще ничего» — с кнопкой «посмотреть,
  что рядом» (требование T7; кнопка идёт через `navigateToTab`, потому что это
  переключение таба, а не переход вглубь); «нет активных, всё в истории» и
  «история пуста» — свои тексты. Предлагать «сходите на главную» тому, у кого
...
- **Переходы — только заказы «Еды»** (`ActivityTarget`, правило то же, что у
  ...
  **корзину еды**, — заказ одежды открылся бы там под видом заказа еды. У
  брони, записи и билета экрана **одной** записи нет (появившиеся в issue #97,
  #99 и #106 «мои записи» и «мои билеты» — это списки, вести строку в другой
  список бессмысленно), и такие строки **не кликабельны**: нажатие без
...
- **Карточка — `OrderCard` из кита эпика 2 для всех пяти источников.**
  ...
  «сколько человек впереди»; ни одного из этих полей контракт не отдаёт. У
  `OrderCard` появился необязательный `onClick` (как у двух его соседей) —
  пустая лямбда оставляла бы рябь нажатия и роль кнопки для TalkBack там, где
  нажатие ничего не делает.
- **Время сортировки — время события**, если источник его даёт: начало брони,
  дата записи. У заказа и билета его нет, там `createdAt`. Дата записи
  собирается из `apptDate` + `startTime` и разворачивается в `Asia/Tashkent`,
  а не в UTC — иначе запись на 09:00 показывалась бы как 14:00, а запись после
  19:00 уезжала бы на следующий день.
...
**Что изменилось при переносе на сегодняшний `main`** (и это главная польза
от того, что PR пролежал неделю):

- **Своих DTO у `ActivityApi` осталось только два.** За неделю в `main`
  появились вертикали, ходящие в те же самые ручки, — и они уже описали
  четыре из пяти ответов: `OrderPageDto` (одежда, issue #108, `GET orders`),
  `AppointmentPageDto` (запись к мастеру, #97, её же переиспользует больница,
  #99), `CinemaTicketPageDto` (кино, #106). Теперь они импортируются, а не
  копируются:
  у бэкенда это одна модель на путь, и вторая копия разъехалась бы с первой
  при первой же правке контракта. Новое здесь только `GamingBooking`.
- **И это сразу починило живой баг.** Своя копия `AppointmentDto` читала
  `startTime` как объект `{hour, minute, second, nano}` — так его описывает
  springdoc. За неделю выяснилось (issue #96, затем #97), что Jackson с
  `JavaTimeModule` шлёт то же поле **строкой** `"14:30:00"`, и в `main`
  появился `core/format/parseServerLocalTime`, принимающий оба вида. На своей
  копии строка уронила бы разбор всей страницы, то есть раздел «Записи к
  мастеру» молча уходил бы в отказ целиком. Случай закрыт тестом
  (`an appointment time is read from a string too`).
- `hasMore` больше не расширение общего `ActivityPageDto<T>` (его не стало) —
  это функция от `last`/`totalPages`: четыре из пяти страниц объявлены в чужих
  вертикалях и общего надтипа не имеют.
- Журнал этапа уехал сюда, в `CHANGELOG.md`, а контракт — в
  `docs/API-CONTRACT.md`: `AGENTS.md` за это время разрезали.

**Что нашло ревью диффа субагентом** (три бага, все чинились здесь же):

- **Пустая вкладка навсегда останавливала пагинацию.** Экран при
  `visible.isEmpty()` рисовал «Faol amallar yo'q» и выходил из блока `return`'ом
  — вместе с пустым состоянием пропадал и хвост списка, а автотриггер догрузки
  живёт именно в нём. Сценарий: нулевая страница `GET orders` пришла из
  двадцати доставленных заказов (`last: false`), активные лежат на первой;
  человек открывал таб, видел «активных нет» и не видел заказа, который прямо
  сейчас готовят. Догрузка при этом не запускалась ни автоматически, ни
  кнопкой. Теперь условие пустой вкладки — `ActivityState.isTabEmpty`, и оно
  требует, чтобы догружать было **нечего**; пока хвост есть, на месте пустого
  состояния стоит он.
- **Полный отказ рисовал шесть блоков ошибки вместо одного.** `sourceFailures`
  писались и при `isTotalFailure`, поэтому «нет сети» давала пять строк
  «Buyurtmalar yuklanmadi», «Kino chiptalari yuklanmadi», … каждую со своей
  кнопкой, и сверх них общий блок ошибки. Плюс те же пять строк висели над
  скелетоном во время «повторить». Теперь при полном отказе отметок разделов
  нет, а `showLoading` их чистит.
- **Догрузка могла воскресить список «до перезагрузки».** `loadMore` редьюсил
  от снимка, снятого **до** suspend'а, а `load()` своего `Job` не хранил и
  встречной защиты не было. Достижимый путь: догрузка упала → в хвосте
  «повторить» → человек ушёл на статус заказа и вернулся → `ScreenResumed`
  гасит `loadMoreFailure`, хвост переключается на крутилку, её
  `LaunchedEffect` тут же просит страницу — параллельно идущей перезагрузке.
  Поздний ответ молча заменял свежий список устаревшим снимком плюс одна
  страница, и смысл `ScreenResumed` терялся без следа. Теперь `load()` держит
  свой `loadJob` (и отменяет предыдущий — «повторить» есть на каждой отметке
  раздела, два тапа давали две параллельные пятёрки запросов), а `loadMore`
  дожидается его и берёт список с курсором **после** него.

Там же — заголовок строки развёрнут: было `«Ovqat buyurtmasi · F-2026-0042»`,
стало `«F-2026-0042 · Ovqat buyurtmasi»`. Заголовок `OrderCard` — одна строка
с многоточием, и на 393 dp обрезался ровно на номере заказа, ради которого
`note` и показывают.

Новые тесты: `ActivityDomainTest` (разбор четырёх наборов статусов, регистр и
разделители, незнакомый статус остаётся активным, вид заказа по вертикали,
ключ строки с источником, порядок обеих вкладок, записи без даты в конце,
стабильность при равном времени, дедупликация склейки, полный и частичный
отказ, «отказали все» при догрузке), `ActivityRepositoryTest` (MockWebServer с
`Dispatcher` по пути — пять запросов идут параллельно, и `enqueue` раздал бы
ответы вразнобой: пути и query, конверт, слияние пяти источников, кликабельна
только «Еда», время брони по началу и фоллбэк на создание, дата записи с
`LocalTime` объектом и строкой в Ташкенте, битое время не выдумывает момент,
запись без id отброшена, активность без полей осталась, один сбойный источник
и 401 у всех, 2xx с `success:false`, три правила `hasMore`, догрузка
спрашивает только заданные источники), `ActivityViewModelTest` (слияние,
частичный отказ не прячет список, полный отказ — единственная ошибка экрана,
пустой ответ не ошибка, фильтр без обращения к сети, пустая вкладка ≠ пустой
экран, догрузка и дедупликация, сбойный источник остаётся в курсоре, частичный
отказ догрузки виден в хвосте, «повторить» не гасит список, перечит на
возврате, pull-to-refresh, переход по заказу еды, молчание на строке без
экрана и на исчезнувшей строке, **недочитанный список не объявляет вкладку
пустой**, **полный отказ не плодит отметки разделов**, **скелетон не висит под
устаревшей отметкой**, **догрузка поверх перезагрузки не воскрешает старый
список**); фейк `FakeActivityRepository` (с воротами, чтобы проверять порядок
ответов); новый случай в `GraphAssemblyTest`.

Каждый из четырёх новых случаев проверен на то, что он **ловит**: правка
временно откатывалась, и все четыре краснели.

**Что изменил третий перенос (9 сентября):**

- **Ушла последняя своя DTO.** Перенос 8 сентября убрал четыре копии из пяти,
  оставив `GamingBookingDto`/`GamingBookingPageDto`: вертикали игровых зон в
  `main` тогда не было. За сутки она приехала (#98, PR #116) — с той же ручкой
  `gaming/bookings/my` и своей копией той же схемы. Теперь `ActivityApi` не
  объявляет **ни одного** DTO: все пять ответов читаются моделями соседних
  вертикалей. В общую `GamingBookingDto` добавлено поле `createdAt` — оно есть
  в схеме стенда, самой вертикали не нужно, а активностям служит фоллбэком
  сортировки, когда не приехал `startTime`.
- **Контракт двух записей разошёлся, и описание было неверным.** Прошлые
  заходы писали, что `appointments/my` и `hospitals/appointments/my` отдают
  одну схему `AppointmentResponse`. Сверка со стендом 9 сентября (`/v3/api-docs`
  отвечает `200`): такой схемы там больше нет вовсе, коллизия springdoc
  разошлась на `AppointmentBookingResponse` у мастера (`placeId`, `serviceId`,
  `serviceName`, `price`, `apptDate`, `startTime`, `endTime`, `status`,
  `createdAt`) и `HospitalAppointmentResponse` у врача (`doctorId`, `apptDate`,
  `startTime`, `complaint`, `status`, `createdAt`). **Код при этом менять не
  пришлось**: обе ручки читаются одним `AppointmentPageDto`, как это уже делает
  сама вертикаль больницы (`HospitalApi`, #99) — поля в нём необязательные, а
  лишние `doctorId`/`complaint` пропускает `ignoreUnknownKeys = true`.
  Практическая разница ровно одна и она новая: **у записи к врачу не будет
  суммы**, бэкенд её в этом ответе не отдаёт. Поправлены `docs/API-CONTRACT.md`
  и комментарии в `ActivityApi`.
- Заодно перепроверено, что `placeName` не появился ни в одной из пяти схем, —
  риск ниже в силе, #150 не закрыт.

**Ревью диффа субагентом в третий заход — ещё два бага, оба починены здесь:**

- **Тест `ActivityRepositoryTest` был гонкой и краснел случайно.** `feed()` шлёт
  пять запросов разом, а MockWebServer держит соединение на поток и зовёт
  `Dispatcher.dispatch` из пяти потоков сразу. Обе карты харнесса были обычными
  `LinkedHashMap`: `requests` теряла записи, а `bodies` писалась из потока теста
  без happens-before к читателям — поток мог не увидеть `respond(...)` и отдать
  пустой конверт по умолчанию. Ревью поймало живое падение
  (`Key /orders is missing in the map`), воспроизводится примерно раз на два
  десятка прогонов. Обе карты — `ConcurrentHashMap`.
- **Страница, целиком отсеянная маппером, навсегда запирала пагинацию.**
  `feed.items.isEmpty()` уводило экран в `ScreenState.Empty`, но курсор при
  этом сохранялся. Хвост списка вместе со своим `LaunchedEffect` рисуется
  только в ветке `Content`, а ручная догрузка выходит по `loaded == null`, —
  значит следующую страницу не попросил бы никто. Сценарий: нулевая страница
  `GET orders` — двадцать записей без `id` (их отбрасывает `mapNotNull`) при
  `"last": false`; человек видит «вы ещё ничего не заказывали», а его заказы
  лежат на первой странице и недостижимы, в том числе после pull-to-refresh.
  Теперь `Empty` требует ещё и `!feed.hasMore`; пустой `Content` при живом
  курсоре показывает хвост, и цепочка догрузки идёт дальше. Пустой вкладкой это
  не объявляется — `isTabEmpty` и так требует отсутствия хвоста. Случай закрыт
  тестом `a fully dropped first page does not end up an empty dead end`,
  проверенным на то, что он **ловит**: с откаченной правкой краснеет.

Что ревью проверило и не нашло претензий: переезд на общую `GamingBookingDto`
(все читаемые поля на месте с теми же `@SerialName`, висячих ссылок на удалённую
копию нет), уникальность ключей `LazyColumn` (`"${source.name}:$id"` — id из
пяти бэкендов не могут столкнуться), гонки `load`/`loadMore`, сохранение
сбойного источника в курсоре, разбор `LocalTime` обоими способами, Hilt-граф.

Что ревью нашло, но здесь **не** чинится — заведено отдельными issue:

- **#174** — сортировка «активных» смешивает прошлое `createdAt` заказов с
  будущим временем броней на одной оси: свежий заказ уезжает под старый.
  Продуктовое решение, не опечатка.
- **#175** — «повторить» и возврат на экран перезагружают с нулевой страницы и
  теряют уже догруженный хвост вместе с позицией прокрутки; заодно охрана
  `ScreenResumed` не смотрит на `isLoadingMore`.
- **#177** — раздел упал, остальные ответили пусто → экран говорит «вы ещё
  ничего не заказывали». Поведение закреплено тестом, то есть выбрано
  осознанно, — переигрывать его в этом PR неправильно.
- **#176** — `parseServerInstant` не принимает ISO-смещения кроме `Z`. Код
  общий, к этому PR отношения не имеет, но `createdAt` у брони его наследует.

Плюс комментарий в **#150** бэкенду: `placeName` перепроверен и не появился, а
...
- **Ни один успешный путь не проверен на живых данных**: все пять ручек
  ...
  всех вертикалей.** Контрактной пробы (`contract/*.sh`, харнесс из
  `contract/booking-pilot`) у активностей нет по той же причине — она упирается
  в отсутствующий `CONTRACT_REFRESH_TOKEN`.
...
- **Единица цены не подтверждена** — тот же риск, что в issue #9: суммы
  приходят целыми числами без дробного близнеца. Считаем сумами. Окажутся
  тийины — суммы будут в сто раз больше.
- **Заголовки строк безымянные**: пока бэкенд не отдаёт название заведения ни
  ...
  `AppointmentBookingResponse` и `CinemaTicket`; она же закрыла бы имя
  заведения на экране статуса заказа (issue #9). Заведена: **issue #150**, там
  же время сеанса у билета и коллизия springdoc у `OrderResponse`. Сверка со
  стендом 9 сентября: `placeName` по-прежнему нет ни в одной из пяти схем,
  **#150 не закрыт**.
- **Список дублирует четыре экрана «моих X»**, появившихся за неделю
  (`MyAppointmentsScreen`, `MyTicketsScreen`, `FashionOrdersScreen`,
  `MyFreelancerOrdersScreen`): те — входы из своих вертикалей, этот — общий
  таб. Данные те же, экраны разные. Сводить их в один — продуктовое решение,
  отдельной задачей.
...
- **Заказы фрилансера (`GET freelancers/orders/my`) в список не включены**:
  их схема — та самая коллизия `OrderResponse`. Своим экраном
  (`MyFreelancerOrdersScreen`) они уже покрыты.
- **Автодогрузка идёт цепочкой, пока хвост списка на экране**: ограничитель
  тут один — то, что хвост уходит из композиции, стоит прокрутить список.
  Если у кого-то история окажется большой, это первое, что стоит заменить на
  кнопку «показать ещё».
- Скриншот-тестов по-прежнему нет: обе вкладки, отметка сбойного раздела и три
  пустых состояния проверялись глазами по `@ThemeLanguagePreviews`. Правило
  «когда показывать пустую вкладку» из-за этого вынесено из композабла в
  `ActivityState.isTabEmpty` — там его хотя бы проверяет тест.
- `ActivityState.visible` — вычисляемый геттер: фильтр и полная сортировка
  считаются заново на каждое обращение, в том числе на рекомпозиции. Пока
  страниц по 20 это неважно; на истории в сотни строк просится `derivedStateOf`
  или кэш в ViewModel.
- `ScreenSkeleton` из кита после удаления `OrdersScreen` не вызывается ниоткуда.
  Оставлен: в ките и так лежат `BookingCard` с `TicketCard` без единого
  вызывающего, а следующая заглушка экрана возьмёт его же.
- `docs/UI-INVENTORY.md` — снимок от 2026-08-30, и он не обновляется по PR'ам
  (строка «Кошелёк — заглушка с выдуманным числом» устарела ещё раньше).
  Поправлена только строка «Заказы», про которую этот PR и есть.
