...
## Сверка бэкенда и Android (issue #92, снимок 2026-09-09)

Кода не менял — только `docs/BACKEND-SYNC.md` и строка в таблице `AGENTS.md`.
Полный разбор по 32 контроллерам — в самом документе, здесь только грабли и
то, чего в нём нет.

- **Прошлый прогон #92 (4 сентября) в `main` не доехал.** Он подготовил тот же
  документ в ветке `claude/issue-92-20260904-0805`, но PR не открывали, и файла
  в репозитории не было. Полагаться на «сверка уже сделана» нельзя: проверять
  надо не отчёт в issue, а `main`. Этот документ написан по свежему снимку с
  нуля, а не дополняет тот.
- **Разрыв впервые сокращается.** 180 ручек у бэкенда, приложение зовёт 77
  (43 %). Было 160/30 (19 %) 4 сентября и 129/12 (9 %) 30 августа. За пять дней
  бэкенд вырос на 20 ручек, приложение подключило 47 — из 16 задач того
  прогона (#93–#108) закрыто 14, открытыми остались #102 (серверный PIN) и
  #105 (соцфункции).
- **Выдуманных путей нет.** Все 77 путей, которые зовёт приложение, есть в
  схеме; проверено сопоставлением Retrofit-аннотаций с `/v3/api-docs` по методу
  и пути с нормализованными `{параметрами}`.
- **Непокрытое — не всё работа Android.** Из 103 оставшихся ручек 35
  пользовательских, 45 — бизнес-панель (эпик #16), 16 — админка, 2 — колбэки
  провайдеров, 1 — алиас OTP, ещё 4 (`food/orders/my`, `fashion/orders/my` и
  их карточки) дырой не являются — заказы всех вертикалей приложение берёт
  одним `GET /orders`. Считать «103 задачи» неверно.
- **Коллизии springdoc бэкенд развёл — все.** Голых `BookRequest`, `Response`,
  `CreateRequest`, `OrderResponse` в схеме больше нет; появились
  `GamingBookRequest` / `AppointmentBookRequest` / `HospitalBookRequest`,
  `ReviewResponse`, `ReviewCreateRequest`, `FoodOrderResponse`,
  `FashionOrderResponse`. Устаревшие записи про эти коллизии лежали не в
  `CLAUDE.md`, а в `docs/API-CONTRACT.md` (две штуки, поправлены в этом же
  коммите) и в KDoc `CatalogApi.kt` / `HospitalApi.kt` / `FashionApi.kt`.
- **Разведённая коллизия вскрыла живой баг (#192).** `ReviewResponse` =
  `{id, placeId, userId, rating, text, isVerified, ownerReply, helpfulCount,
  createdAt}` — ни `userName`, ни `userAvatarUrl` в нём нет, а `ReviewDto`
  разбирает их под шестью именами через `@JsonNames`. Дефолты не дают разбору
  упасть, поэтому имя автора отзыва **всегда пустое, а аватар всегда `null`** —
  и это никем не замечено. Грабли на будущее: алиасы «на случай коллизии»
  ошибку не чинят, а прячут; когда схему разводят, их надо снимать сверкой.
  Нашёл это ревьюер-субагент, а не сам прогон.
- **Два комментария в коде устарели наполовину.** `feature/food/domain/Cart.kt`
  и `feature/promotions/ui/PromotionCard.kt` пишут, что стоимости доставки и
  промокода у бэкенда нет. Доставка появилась
  (`GET food/delivery-fee?itemsAmount=` → `{"deliveryAmount":10000}`,
  проверено curl'ом), промокод — только для «Одежды»: в
  `FashionPlaceOrderRequest` поле `promoCode` есть, в `FoodPlaceOrderRequest`
  нет. Задачи #179 и #180; расхождение внутри бэкенда — в #191.
- **Гейт `403 GEO_PERMISSION_REQUIRED` теперь и на публичных ручках** — без
  `X-Geo-Lat`/`X-Geo-Lng` не отвечает даже `GET /cinema/movies`. Приложение это
  уже закрывает `GeoHeaderInterceptor` (#53), дыры нет, но снимать контракт
  curl'ом без этих заголовков больше нельзя — легко принять гейт за поломку.
- **Запись к врачу уходит вслепую.** `GET hospitals/doctors/{id}/slots?date=`
  существует и не зовётся ни разу: время приёма пользователь называет сам.
  `hospital-controller` вырос с 4 ручек до 10. Задача #181.
- **`GET /places?ids=` снимает клиентскую часть #150** — отдаёт `Summary` с
  `name`, то есть названия заведений в «Моих активностях» можно резолвить
  пачкой, не дожидаясь `placeName` от бэкенда. Задача #182.
- **`GET /places/my` без токена по-прежнему `500`, а не `401`** — замечание
  4 сентября воспроизведено. Все остальные защищённые ручки отвечают `401`
  корректно.
- Заведено 14 задач: #179–#185 (пользовательские), #186–#190 (бизнес-панель),
  #191 (NEEDS-PARTNER бэкенду), #192 (баг с автором отзыва). Дубликатов не
  создавал — где задача уже была (#102, #105, #167–#170), в документе стоит
  ссылка на неё.
- Тесты и сборка прогнаны, хотя менялись только документы: `AGENTS.md` требует
  зелёных `testDebugUnitTest` и `assembleDebug` до пуша без исключений.
  `BUILD SUCCESSFUL`, 1842 теста в 177 классах, ноль провалов.
