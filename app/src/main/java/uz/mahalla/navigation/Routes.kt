...
/**
 * Настройки уведомлений (эпик 11): категории и тихие часы. Вне графа табов,
 * как и сам центр уведомлений, — открываются и из него, и строкой из профиля.
 *
 * Отдельный экран, а не раздел профиля: категорий шесть, к ним тихие часы и
 * карточка разрешения, — в профиле, где уже язык, тема, устройства и десяток
 * строк «мои …», всё это утонуло бы.
 *
 * Аргументов нет: настройки локальные и читаются из DataStore самим экраном.
 */
@Serializable
 ...
 *
 * Тот же экран работает и **переносом** записи ([rescheduleId] задан): выбирают
 * там ровно то же — день и слот, — а второй экран с календарём и слотами
 ...
 * @param serviceId выбранная заранее услуга; пусто — выбирает человек. Задаётся
 * при переносе: услуга остаётся та же, меняется только время.
 * @param rescheduleId переносимая запись. Пусто — обычная запись с нуля.
