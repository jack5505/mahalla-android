 ...
 * побайтовая копия свелась к этой). `ApiErrorState` тут не годится: он
 * подменяет собой весь контент, а здесь отказать должен только фрагмент —
 * один из нескольких независимых источников на экране (форма, точечное
 * действие, один из списков) — пока остальной экран остаётся рабочим.
 */
@Composable
fun InlineFailure(
    failure: ApiFailure,
