 ...
 *    сервере, который отдаёт 401 на валидный токен. Но и этот отказ не
 *    молчит бесследно: набирается ли за ним отказ токена по нескольким
 *    разным ручкам — см. `onFreshTokenStillRejected` (issue #197).
 *
...
class TokenAuthenticator @Inject constructor(
    ...
    /**
     * Разные ручки (`метод путь`), чей повтор с только что обновлённым
     * токеном (см. [attemptCount]) всё равно получил 401 — issue #197.
     * Читается и пишется только внутри `synchronized(this)`.
     *
     * Одна ручка, путающая 401 и 403 (роль, подписка, бизнес-панель), не
     * должна рвать сессию — иначе выкидывает из приложения человека, у
     * которого всё остальное работает. Но если РАЗНЫЕ ручки подряд не
     * принимают один и тот же свежий токен, единственная одноручечная причина
     * уже не объясняет происходящее.
     *
     * Обнуляется вместе с остальным в [reset] — тем же вызовом, что и после
     * каждого удачного refresh. Это не случайность: пока набор не сброшен
     * удачным refresh'ем, следующая ручка добралась сюда через «токен уже
     * обновлён параллельным запросом» (ветка `session.accessToken !=
     * staleToken` ниже) — то есть несколько по-настоящему одновременных
     * запросов делят один и тот же свежий токен, и совместный отказ — это и
     * есть системный отказ токена. Если же между двумя отказами случился
     * СВОЙ собственный удачный refresh (запрос пришёл уже с текущим токеном,
     * а не с устаревшим), [reset] стирает набор до записи следующей ручки —
     * и утренняя бизнес-панель без подписки с вечерней платной ручкой не
     * сложатся и здоровую сессию не порвут.
     */
    private val pathsRejectingFreshToken = mutableSetOf<String>()

    /**
     * Обнулить счётчик неоднозначных провалов refresh и набор
     * [pathsRejectingFreshToken]. Сессию пишут и стирают ещё в нескольких
     * местах мимо этого класса — `AuthRepository.signIn()` (а значит и
     * повторный вход после `logout()`/`clearLocalIdentity()`). Без сброса
     * застрявший счёт из прежней сессии добивает себя до порога первым же
     * ответом в новой и стирает её мгновенно — тот же цикл «вход → платный
     * SMS → моментальный выход», от которого защищает issue #138
     * (issue #309).
     */
    fun reset() {
        synchronized(this) {
    ...
    override fun authenticate(route: Route?, response: Response): Request? {
        if (attemptCount(response) >= MAX_ATTEMPTS) {
            return synchronized(this) { onFreshTokenStillRejected(response) }
        }
    ...
    /**
     * Запрос со свежим токеном (см. [attemptCount]) всё равно получил 401.
     * Повторить нечем — этому конкретному запросу OkHttp здесь и так не даст
     * попытки, — но стоит понять, одна это неаккуратная ручка (403 вместо
     * 401, простить) или отказ токена целиком по нескольким ручкам сразу
     * (issue #197). Вызывать только внутри `synchronized(this)`.
     */
    private fun onFreshTokenStillRejected(response: Response): Request? {
        val path = response.request.let { "${it.method} ${it.url.encodedPath}" }
        pathsRejectingFreshToken.add(path)
        if (pathsRejectingFreshToken.size >= REJECTING_PATHS_LIMIT) {
            // Несколько разных ручек не приняли один и тот же свежий токен —
            // одноручечное объяснение (401 вместо 403 на роли/подписке) сюда
            // больше не подходит. Ошибиться в сторону «стереть» по-прежнему
            // дорого (issue #138), но молчать здесь — оставить человека перед
            // кнопкой «повторить», которой уже нечем помочь (issue #197).
            runBlocking { sessionStore.clear() }
            sessionExpiry.notifyExpired()
            reset()
        }
    ...
    companion object {
        ...
        /**
         * После скольких РАЗНЫХ ручек, чей повтор со свежим токеном получил
         * 401, считать сессию мёртвой (issue #197). Одну прощаем — это может
         * быть ручка, путающая 401 и 403 (роль, подписка, бизнес-панель), а
         * не отказ токена; но вторая другая ручка подряд (без своего
         * успешного refresh между ними, см. [pathsRejectingFreshToken]) уже
         * не объясняется одной кривой ручкой.
         *
         * Эвристика без данных с прод-стенда (`SENTRY_DSN` не настроен) —
         * то же ограничение, что и у [MAX_AMBIGUOUS_REFRESH_FAILURES].
         */
        const val REJECTING_PATHS_LIMIT = 2
