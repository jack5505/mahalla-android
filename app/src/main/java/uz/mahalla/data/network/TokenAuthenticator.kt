...
class TokenAuthenticator @Inject constructor(
     ...
     * (а значит и повторный вход после `logout()`/`clearLocalIdentity()`) и
     * `SecurityRepository.resumeSession()` (продолжение той же сессии после
     * блокировки приложения). Без сброса застрявший на 1–2 счёт из прежнего
     * состояния добивает себя до порога первым же неоднозначным ответом и
     * стирает свежую сессию мгновенно — тот же цикл «вход → платный SMS →
     * моментальный выход», от которого защищает issue #138 (issue #309,
     * issue #363).
     */
    fun reset() {
        synchronized(this) {
