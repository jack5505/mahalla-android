...
/**
 * `WalkInResponse` walk-in-контроллера. Все поля необязательные: отсутствие
 * любого из них — не повод показать ошибку вместо талона.
 *
 * Раньше это имя в `/v3/api-docs` называлось просто `Response` и было
 * перекрыто коллизией springdoc (под ним были склеены walk-in, `place-staff`
 * и `reviews`), а показанный вариант случайно совпадал с walk-in — поля
 * здесь были читаемы, в отличие от отзывов (issue #76). Сверка 2026-09-10
 * разнесла имена по вертикалям (issue #235): своя `WalkInResponse` — та же
 * схема, что использует бизнес-панель для очереди (`BusinessApi.queue`,
 * issue #96), хоть панели и нужен другой набор полей (см. `QueueEntryDto`).
 *
