 ...
 * тело раньше было объявлено под именем `CreateRequest`, перекрытым коллизией
 * springdoc — под ним лежали пять разных запросов, и показанный вариант
 * случайно совпадал с отзывом (`placeId` + `rating`). Сверка 2026-09-10
 * разнесла имена по вертикалям: тело этого пути теперь называется
 * `PlaceCreateRequest` (issue #235), но собственные поля этой схемы
 * по-прежнему не перечитаны — `401` приходит до валидации, а
 * `CONTRACT_REFRESH_TOKEN` в проекте всё ещё нет. Поэтому поля остаются
 * названы так же, как в ответе `Detail`, который бэкенд отдаёт на этот же
 * эндпоинт (`name`, `category`, `description`, `address`, `lat`, `lng`,
 * `city`, `phone`, `website`) — то же решение, что принято для отзывов в
 * issue #76.
 */
