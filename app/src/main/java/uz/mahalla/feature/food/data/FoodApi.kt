 ...
 * суммами и без коллизий ни с одной вертикалью. У второго имя схемы
 * (`OrderResponse`) раньше в `/v3/api-docs` перекрывала коллизия springdoc
 * (под ним лежал заказ фрилансера); сверка 2026-09-10 разнесла имена по
 * вертикалям — у еды теперь своя `FoodOrderResponse` (issue #235). Решение не
 * изменилось: `OrderView` уже даёт все суммы без второго парсера под ответ,
 * который тут же перечитывают.
 */
interface FoodApi {
    ...
    /**
     * Отмена. Тело ответа не разбирается: имя схемы (`FoodOrderResponse`)
     * больше не перекрыто коллизией (issue #235, сверка 2026-09-10), но
     * состояние заказа всё равно перечитывается [order]'ом — у него уже есть
     * выверенная `OrderView`, а второй разбор ради тех же данных добавил бы
     * риск без выгоды: догадываться о полях там, где ошибка превратит
     * удачную отмену в «не удалось», незачем.
     */
