...
interface OrderRepository {
    ...
    /**
     * Оформление. Возвращается только идентификатор заказа: имя схемы ответа
     * (`FoodOrderResponse`) больше не перекрыто коллизией springdoc (issue
     * #235, сверка 2026-09-10), но состав и суммы экран статуса всё равно
     * берёт у `GET orders/{id}` — там схема `OrderView` общая для всех
     * вертикалей и без коллизий, второй парсер под ответ создания незачем.
     *
