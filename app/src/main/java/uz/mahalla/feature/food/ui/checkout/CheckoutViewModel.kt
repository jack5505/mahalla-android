 ...
 *
 * Полей времени и комментария на экране нет: бэкенд их не принимает
 * (issue #63), а форма, которая собирает молча выбрасываемые данные, обманывает
 * человека дважды — при заполнении и при получении заказа.
 *
    ...
    private fun submit() {
        ...
        viewModelScope.launch {
            when (val result = orderRepository.create(cart.copy(lines = state.lines), state.form)) {
                is ApiResult.Failure -> updateState {
                    // Именно `failure`, а не `error`: текст, который прислал
                    // сервер, человеку полезнее нашей общей формулировки
                    // (issue #34).
