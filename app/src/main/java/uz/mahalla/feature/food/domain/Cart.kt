 ...
 *
 * Доставку до оформления называет `GET food/delivery-fee` по сумме позиций
 * (issue #179) — в корзине и чекауте это **оценка**, и она показывается только
 * при доставке: у самовывоза строки нет. Окончательные суммы всё равно считает
 * сервер, поэтому итог оформленного заказа берётся из ответа о заказе
 * (`OrderView`), а не отсюда.
 *
 * Скидка остаётся нулём: промокод приложить к заказу «Еды» нечем — поля под
 * код в `PlaceOrderRequest` нет (issue #180).
 */
...
object CartCalculator {
     ...
     * на клиенте нечем — доставку называет `food/delivery-fee`, скидку и
     * окончательный итог сервер в ответе о заказе.
     */
    fun totals(
        lines: List<CartLine>,
        discountSum: Long = 0,
