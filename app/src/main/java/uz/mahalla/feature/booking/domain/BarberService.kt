 ...
 *
 * @param priceSum цена, **как её отдал сервер, без пересчёта**. Ноль — «цена
 * не названа»: экран тогда её просто не показывает, а не пишет «0 сум».
 *
 * **Единица подтверждена бэкендом, и имя поля больше не соответствует
 * содержимому.** `info.description` живого `/v3/api-docs` (сверено 2026-09-10)
 * говорит прямо: все целые денежные поля — тийины, 1 сум = 100 тийин. Стенд
 * отдаёт за стрижку `5000000`, то есть 50 000 сум, а экран рисует
 * «5 000 000 so'm» — в сто раз больше. Делитель здесь наугад не вводим:
 * правка сквозная, все вертикали разом — issue #149, см.
