 ...
 *
 * Тот же домен переиспользует и вертикаль мастеров (`FreelancerApi.services`,
 * issue #216): своя схема (`FreelancerServiceResponse`), но набор полей на
 * экране совпадает, а `freelancerId` там не нужен — маппер получает его
 * снаружи, из аргумента запроса.
 *
 * @param priceSum цена в **сумах**. Бэкенд отдаёт `price` в тийинах (issue
 * #149: стенд шлёт за стрижку `5000000` — это 50 000 сум), пересчёт делает
 * маппер (`Money.tiyinToSom`), экран единицу бэкенда не знает. Ноль — «цена
