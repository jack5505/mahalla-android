 ...
 *
 * @param priceSum цена. Бэкенд отдаёт `price` целым числом **без** дробного
 * близнеца (в кошельке пара `balance`/`balanceSom` есть — issue #62, здесь
 * нет), поэтому считаем сумами, как в «Еде» (issue #9). Ноль — «цена не
 * названа»: экран тогда её просто не показывает, а не пишет «0 сум».
 *
 * **Единица не подтверждена.** Стенд отдаёт за стрижку `5000000`: как сумы
 * это неправдоподобно, как тийины — ровно 50 000 сум. `WalletAmounts` из
 * такого же отсутствия дробного поля делает противоположный вывод (тийины).
 * Делитель наугад не меняем — нужен ответ бэкенда, см. `docs/API-CONTRACT.md`.
