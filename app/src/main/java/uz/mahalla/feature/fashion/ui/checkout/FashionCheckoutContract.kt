 ...
 *
 * Форма и её правила — те же, что у «Еды»: `CheckoutForm`/`CheckoutError`
 * переиспользуются, чтобы валидатор не разошёлся в двух местах. Схема запроса
 * на бэкенде своя (`FashionPlaceOrderRequestDto`, issue #221) — состав заказа
 * эта форма не описывает, его сервер берёт из корзины.
 *
 * @param items строки **одного магазина**: серверная корзина общая, а заказ
 * оформляется по одному `storeId` за раз.
