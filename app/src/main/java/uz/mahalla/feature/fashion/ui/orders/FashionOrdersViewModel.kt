 ...
 * схема ответа (`FashionOrderResponse`) больше не перекрыта коллизией
 * springdoc (issue #235), решение не изменилось (см. `FashionApi`).
 *
...
class FashionOrdersViewModel @Inject constructor(
     ...
     * на клиенте: имя схемы ответа `POST fashion/orders/{id}/cancel`
     * (`FashionOrderResponse`) больше не перекрыто коллизией (issue #235), но
     * решение не изменилось — тело всё равно не разбирают. Не удалось
