 ...
 *
 * **Теперь бэкенд её описал** (сверено 2026-09-10, issue #154):
 * `info.description` в `/v3/api-docs` говорит, что все целые денежные поля —
 * тийины. Значит вывод масштаба ниже больше не нужен и [scaleOf] должен стать
 * жёсткой сотней: сейчас он может вернуть `1` и тем разойтись с контрактом.
 * Правка идёт вместе с остальными вертикалями — issue #149, здесь не в
 * одиночку: кошелёк единственное место, где пара полей даёт проверку, и ломать
 * её до общей правки незачем.
 *
