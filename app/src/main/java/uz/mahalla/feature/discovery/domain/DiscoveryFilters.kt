...
data class DiscoveryFilters(
    ...
    /**
     * Убирает из применённого фильтра категории, которых больше нет в
     * актуальном списке (issue #382): дашборд мог выключить категорию уже
     * после того, как её выбрали, — чип пропадает из шторки, а фильтр без
     * этого должен был бы остаться приклеен к невидимой больше категории.
     */
    fun restrictedTo(available: Collection<PlaceCategory>): DiscoveryFilters =
