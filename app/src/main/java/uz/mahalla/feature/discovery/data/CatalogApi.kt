 ...
 *
 * Схема стенда раньше называла этот класс `CreateRequest`, и под тем же
 * именем springdoc склеивал пять разных запросов (отзыв, заведение, акция,
 * товар аптеки, фрилансер) — показанный вариант случайно описывал как раз
 * отзыв (`placeId` + `rating` + `text` + `appointmentId`). Сверка 2026-09-10
 * разнесла имена по вертикалям: тело теперь называется `ReviewCreateRequest`
 * (issue #235). Собственные поля этой схемы по-прежнему не перечитаны:
 * `POST reviews` без токена отвечает `401 UNAUTHORIZED` ещё до валидации, а
 * токен в CI взять негде.
 *
