...
private fun LazyListScope.activityItems(
    ...
    when (val items = state.items) {
        ...
        is ScreenState.Content -> {
            // Активности есть, но не в этой вкладке. Предлагать «сходите на
            // главную» тому, у кого двадцать заказов в истории, значит не
            // заметить его самого. Условие — в `ActivityState.isTabEmpty`:
            // оно не такое очевидное, каким кажется, и там его проверяет тест.
            if (state.isTabEmpty) {
...
private fun ActivityRow(
    ...
    OrderCard(
        order = OrderCardUi(
            ...
            // (только `placeId`), поэтому заголовок — то немногое, что он даёт
            // словами: номер заказа, услуга, место в зале, — а за ним вид
            // активности. Именно в таком порядке: заголовок `OrderCard` — одна
            // строка с многоточием, и «Ovqat buyurtmasi · F-2026-0042» на
            // 393 dp обрезается ровно на номере, ради которого его и показывают.
            title = activity.note?.let { "$it · $kindLabel" } ?: kindLabel,
