 ...
 * Только **частичный** отказ: при полном карта пуста, иначе рядом с общей
 * ошибкой экрана встали бы ещё пять строк про тот же 401. Пуста она и пока
 * список в `Loading` — отметка держится на списке, который объясняет.
