...
data class ActivityState(
    ...
    /** Хвост списка: догрузка идёт или её надо повторить. */
    val hasTail: Boolean get() = hasMore || loadMoreFailure != null

    /**
     * Показывать ли «в этой вкладке пусто».
     *
     * Только когда догружать больше нечего. Вкладку отбирает клиент, и первая
     * страница источника может целиком уехать в соседнюю вкладку — сказать
     * «активных нет», не дочитав список, значит спрятать заказ, который прямо
     * сейчас готовят. Пока есть [hasTail], на месте пустого состояния стоит
     * хвост, он же и тянет следующую страницу.
     */
    val isTabEmpty: Boolean
        get() = items is ScreenState.Content && visible.isEmpty() && !hasTail
}
