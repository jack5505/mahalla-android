 ...
 * Само значение живёт в DataStore (`PinAttemptStore`), а не в этом состоянии:
 * счётчик в памяти обходился снятием приложения из недавних.
...
sealed interface AppLockEffect : UiEffect {
     ...
     * блокировки, за которым нет сессии, это тупик.
     *
     * Замок к этому моменту **ещё вооружён** и оверлей ещё на экране: снять
     * его раньше значило бы отменить коллектор, который этот эффект и должен
     * получить. Разбирает замок получатель — `RootViewModel`.
     */
