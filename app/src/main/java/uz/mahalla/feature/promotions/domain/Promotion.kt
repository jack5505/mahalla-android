 ...
 * заявки заведения приходилось выводить из ответа (issue #84, разведена
 * сверкой 2026-09-10 — issue #235), здесь нет и не было.
 *
 * Суммы — целые сумы. Бэкенд отдаёт их в тийинах, пересчёт `Money.tiyinToSom`
 * делает маппер (issue #149).
 */
...
/**
 * Результат проверки промокода (`GET promotions/check`, issue #180).
 *
 * Сумы — пересчёт из тийинов делает маппер, как у [Promotion]. [finalAmount] —
 * подсказка для показа **до** оформления; итог созданного заказа сервер
 * считает заново и может отличаться (доставка, изменившаяся корзина), поэтому
 * после `POST fashion/orders` берётся сумма из ответа заказа, а не отсюда.
 */
data class PromoCheckResult(
    /** Код, который подтвердил сервер; при его молчании — тот, что вводили. */
    val code: String,
    val valid: Boolean,
