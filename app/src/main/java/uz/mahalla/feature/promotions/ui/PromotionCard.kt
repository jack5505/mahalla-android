...
fun PromotionCard(
    ...
    MahallaCard(
        ...
        Column(
            ...
            // Код показываем как есть: карточка общая для всех вертикалей, а
            // применить его прямо отсюда можно только в чекауте «Одежды»
            // (issue #180) — у остальных поля под код в заказе нет.
