...
interface CinemaApi {
     ...
     * гостевого режима не знает — граф до него доходит только через
     * `OnboardingGraph` (телефон, OTP), поэтому противоречия с анонимной
     * афишей нет, хотя сам маршрут и не лежит внутри `navigation<MainGraph>`
     * (тот несёт только четыре таба).
     */
