...
/**
 * `FreelancerOrderResponse` (сверено issue #235, 2026-09-10).
 *
 * Раньше это имя в схеме называлось `OrderResponse` и было перекрыто
 * коллизией springdoc — на `ApiResponseOrderResponse` ссылались заказы еды,
 * одежды и мастеров одновременно, и в показанном варианте оказались именно
 * поля мастера: `freelancerId`, `serviceId` и `serviceTitle`, которых у
 * заказа еды быть не может. Сверка разнесла имена по вертикалям: у мастеров
 * теперь своя `FreelancerOrderResponse`, и поля ниже — не угаданный
 * «победитель коллизии», а собственная схема мастера.
 *
