...
class AppLockManagerTest {
    ...
    /**
     * Холодный старт — та же длинная отлучка.
     *
     * Отметка фона живёт в памяти процесса, поэтому снятое из недавних или
     * убитое системой приложение приходит на передний план без неё. Считать
     * это «вернулись мгновенно» значит открывать приложение перезапуском —
     * самым обычным способом «долго в него не заходить». Вместе с замком тогда
     * обесценивается и лимит попыток: подбирать код незачем.
     */
    @Test
    fun `a cold start locks the app just like a long absence`() = runTest {
        val manager = manager()

        // Фона до этого не было — процесс только что создан.
        manager.onForeground()
    ...
    @Test
    fun `a cold start without anything to unlock does not lock`() = runTest {
        sessionStore.clear()
        val manager = manager()

        manager.onForeground()

        // Замок, который открыть нечем, и есть запирание вне приложения.
        assertFalse(manager.locked.value)
    }

    @Test
    fun `a second foreground after an unlock changes nothing`() = runTest {
        val manager = manager()
        manager.onForeground()
        manager.unlock()

        // Оверлей сняли и тут же вернулись из короткой отлучки: замок больше
        // не защёлкивается, отметка холодного старта уже израсходована.
        manager.onBackground()
        clock.advance(Duration.ofSeconds(5))
        manager.onForeground()
