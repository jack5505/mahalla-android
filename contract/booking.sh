...
# И главное — в BookingApi.kt имена полей `BookAppointmentRequest` были
# выведены, а не прочитаны: схема `BookRequest` была перекрыта коллизией
# springdoc. Коллизии в схеме больше нет, и `AppointmentBookRequest` имена
# подтверждает (issue #167), но что бэкенд сделает с запросом, без токена
# по-прежнему не узнать: 401 приходит до валидации.
#
#   contract/booking.sh
#
