...
`app-release-unsigned.apk` — 108 МБ, из них ~103 МБ — нативные библиотеки
Yandex MapKit на четыре ABI (26 МБ на arm64). Кода после R8 всего 6 МБ.
Пользователь столько не качает: Play отдаёт AAB и режет по ABI, то есть
загрузка — порядка 30 МБ. Это оценка по размеру одной ABI, а не измерение:
`bundleRelease` пока никто не гонял. Универсальный APK такого размера —
нормальный результат, а не регресс; ужимать имеет смысл только сам MapKit.
...
Хранилища нет — release собирается **неподписанным**. Так сделано намеренно:
`assembleRelease` в CI проверяет, что отработал R8, и ронять эту проверку из-за
отсутствия ключа неправильно — ровно так один незаполненный секрет ломал бы
сборку всем, включая форки.

Признак того, что ключа не было, — **имя файла**: `app-release-unsigned.apk`
вместо `app-release.apk`. Предупреждения в логе при незаданном
`MAHALLA_KEYSTORE_FILE` не будет; `logger.warn` срабатывает только когда
переменную задали, а файла по этому пути нет. Смотрите на имя, а не на лог:
неподписанный APK в Play всё равно не примут, но заметить это лучше раньше.
...
## Как сборка попадает к тестировщикам сейчас

Единственный работающий путь раздачи — workflow **Release (internal test)**
(`.github/workflows/release-internal.yml`): Actions → Run workflow или тег
`v1.2.3`. Он гоняет тесты, собирает **debug**-APK (подписан отладочным ключом,
ставится без настройки подписи) и кладёт его в GitHub Release. Это не
магазинная сборка: R8 в ней выключен, адрес бэкенда — стенд.

Release-сборка из этого документа — путь в Play, и он пока ручной.
