...
- **Редизайн «Focus» (PR #283) опережает бэкенд в двух местах**, и это
  зафиксировано issue, а не додумано: плитка карты на карточке места и
  карточка номера талона (#286 — нет `MAPKIT_API_KEY` и нет номера талона от
  бэкенда). `core/format/TicketFormatter.kt` («A-042», эпик 1.5) с тех пор не
  используется нигде — мёртвый код до решения по #286. Бронирование шторкой
  (#285) решено **не** делать: `navigation-material` архивен и не тянется
  ради одного экрана, а подъём состояния в `PlaceDetailsScreen` ломает
  перенос записи (там нет карточки места вовсе) — см. [ADR
  0011](adr/0011-bronirovanie-ostaetsya-ekranom.md). Бейдж талона на табе
  «Мои активности» (#284) решён осознанным расхождением с макетом, а не
  додумыванием бэкенда — [ADR 0012](adr/0012-badge-talona-ne-na-tabe.md):
  талона нет в данных «Моих активностей», и заводить его туда шестым
  источником раньше времени, пока открыты #208/#213 (пагинация того же
  списка), не стали — см. #287.
