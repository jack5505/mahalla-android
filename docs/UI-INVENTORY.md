...
**A3. Таб «Заказы» → «Мои активности».** — **сделано, issue #73.**
...
*Сделано с одним отличием от текста выше: заказы читаются общей ручкой
`GET orders`, а не `food/orders/my` — у неё схема `OrderView` однозначна, а
приезжают заказы всех вертикалей сразу. Подробности — в `CHANGELOG.md` и
