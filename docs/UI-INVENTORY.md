...
3. ~~**Каталог стенда пуст**: `places/nearby`, `search` и `map-bounds` отвечают
   `200` с `data: []` на любой радиус (проверено в issue #53).~~ Наполнен:
   на 2026-09-10 `nearby` и `map-bounds` по центру Ташкента отдают 25
   заведений с координатами (issue #168).
4. ~~**Карта — заглушка со списком маркеров.** Полотно `MapCanvas` (Yandex
   ...
   по-прежнему рисует `MapCanvasPlaceholder`.~~ Подключено в issue #65
   (`MapCanvasPlaceholder` удалён), маркеры грузятся по видимой области в
   issue #168. Остаётся секрет `MAPKIT_API_KEY`: без него на месте карты
   объяснение, а не тайлы.
...
**B1. Карта по-настоящему.** ✅ Сделано: полотно подключено (issue #65),
маркеры грузятся по видимой области `GET places/map-bounds` с дебаунсом
(issue #168), сеточный `MarkerClusterer` удалён — кластеризует сам MapKit
(ADR 0002). Осталось: нужен секрет `MAPKIT_API_KEY` в Actions —
**действие пользователя**, без него на месте карты объяснение, а не тайлы.
