...
| Заказы | ~~`feature/orders/ui/OrdersScreen.kt`~~ → `feature/activity/ui/ActivityScreen.kt` | **сделано** (issue #73, задача A3): «Мои активности» — список из пяти источников, фильтр «активные/история», частичный отказ, догрузка по кнопке «Показать ещё» (#151) |
...
**A3. Таб «Заказы» → «Мои активности». Сделано** (issue #73, дедупликация
контракта — #142).
Один список из пяти источников. Заказы читаются **общей** ручкой `GET orders`
без `vertical` (у `food/orders/my` схема перекрыта коллизией springdoc), плюс
`GET gaming/bookings/my`, `GET appointments/my`,
`GET hospitals/appointments/my`, `GET cinema/tickets/my`. Фильтр
«активные/история», частичный отказ, догрузка по каждому источнику отдельно
(по кнопке «Показать ещё» — issue #151), переход на статус заказа «Еды».
