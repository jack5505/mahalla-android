...
все 81 путь клиента существуют на стенде (сверено с живым `/v3/api-docs`
2026-09-09; счётчик пересчитан 2026-09-10 — от 77 он разошёлся ещё до
`food/delivery-fee`, см. таблицу замеров). Работа сместилась с «написать
вертикаль» на **платформенные куски** — соцфункции, серверный PIN, платежи,
push, бизнес-панель, — и почти каждый из них **уже лежит в открытом PR**, а
не ждёт исполнителя.
...
- **Роли**: «кто вы» → анкета покупателя / продавца (#84), «мои заведения» со
  ...
  Владелец/менеджер прямо со списка «мои заведения» правит витрину аптеки
  (#252, аптека) и заводит акцию любой категории заведения (#252, акция) —
  без захода в неслитую бизнес-панель (PR #161).
...
Из 180 эндпоинтов стенда приложение использует 81 (пересчёт 2026-09-10).
Разница — 99 ручек, и это **не** «отставание на порядок»: около 63 из них —
бизнес-панель, кабинет мастера и админка. Клиентские незакрытые области — десять, и пять из них
уже в открытых PR:
...
- **`promotions/check`** — промокода в чеке нет: приложить код к заказу «Еды»
  нечем (issue #180). Стоимость доставки отсюда ушла — `food/delivery-fee`
  подключён (issue #179).
...
- **`.claude/rules/testing.md` называет 163 тест-класса** — их 190 (замер 2026-09-10). Правка
...
- **Редизайн «Focus» (PR #283) опережал бэкенд в трёх местах**, и это было
  зафиксировано issue, а не додумано. Бейдж талона на табе (#284) всё ещё
  ждёт: талона нет в данных «Моих активностей». #286 закрыт частично: плитка
  карты на карточке места сделана (`PlaceMapTile`, `MapCanvas` переиспользован
  с выключенными жестами SDK, без ключа — объяснение вместо тайлов); номер
  талона («A-15») бэкенд по-прежнему не отдаёт (только `queuePosition`) —
  `core/format/TicketFormatter.kt` («A-042», эпик 1.5) удалён как мёртвый код,
  а не оживлён, чтобы не выдумывать несуществующий контракт. Бронирование
  шторкой (#285) решено **не** делать: `navigation-material` архивен и не
  тянется ради одного экрана, а подъём состояния в `PlaceDetailsScreen` ломает
  перенос записи (там нет карточки места вовсе) — см. [ADR
  0011](adr/0011-bronirovanie-ostaetsya-ekranom.md).
...
7. **Очередь из ТЗ**: похоже, это `walk-in` — подтвердить, что других
   эндпоинтов очереди/талонов не планируется и приедет ли когда-нибудь номер
   талона (`A-15` в макете, `queuePosition` в ответе сейчас — см. #286).
