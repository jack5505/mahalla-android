...
`app/src/main/java/uz/mahalla/data/network/analytics/AnalyticsApi.kt` — два
пути с разным устройством отправки.

| Метод | Путь | |
|---|---|---|
| POST | `analytics/track` | ✅ путь и схема сверены со стендом 2026-09-10 (`/v3/api-docs` + curl'ы, issue #169), `401` без токена; успешное тело под токеном не проверено — `CONTRACT_REFRESH_TOKEN` не задан |
| POST | `analytics/events` | ⚠️ контракт получен от бэкенда в комментарии issue #226 (2026-09-19, `jack5505/mahalla#217`), на стенде **не проверен** — на момент внедрения дев-сервер не выкатан (у бэкенда кончились минуты Actions) |

### `track` — счётчик взаимодействий с заведением
...
конверт без полезной нагрузки. `BOOK` и `ORDER` в приложении шлются на все
вертикали сразу, а различает их `metadata.vertical` (`food`, `fashion`,
`queue`, `booking`, `hospital`, `gaming`, `cinema`) — по заявлению бэкенда
(issue #226, тот же комментарий) `metadata` раньше сохранялась, но не
попадала в БД, и это уже починено; на стенде это отдельно не проверялось.

Три факта, из которых следует устройство отправки:

- **токен обязателен.** Аноним с гео-заголовками получает
  `401 UNAUTHORIZED`, без гео-заголовков — `403 GEO_PERMISSION_REQUIRED`.
  Каталог смотрят и до входа, поэтому без сессии клиент запрос **не делает**.
- **батчинга нет** — одно событие, один запрос.
- **поля времени события в запросе нет** — момент события для бэкенда это
  момент, когда запрос доехал до сервера.

`lat`/`lng` в теле объявлены, но клиент их **не заполняет**: координаты уже
уходят в `X-Geo-Lat`/`X-Geo-Lng`. Берёт ли бэкенд гео `track` из заголовков —
по-прежнему не подтверждено (допущение, issue #226).

### `events` — событие без обязательного заведения (issue #226)

Закрывает то, чего не мог `track`: экран без заведения, поисковый запрос, шаг
воронки до выбора заведения, отказ бэкенда как продуктовое событие, событие
до входа.

```json
AnalyticsEventsRequest: {
  "deviceId": "8f3c…",             // обязателен без токена, слать всегда
  "events": [
    {
      "name": "screen_view",       // свободная строка, ≤ 64, [A-Za-z0-9_.:-]
      "occurredAt": "2026-09-19T08:15:30Z", // ISO-8601 UTC, клиент задаёт сам
      "placeId": null,             // необязателен, UUID
      "metadata": {"screen": "wallet"} // необязателен, ≤ 4 КБ, не валидируется
    }
  ]
}
```

Ответ: `{"success":true,"data":{"accepted":1},"timestamp":"…"}`. `accepted`
может быть **меньше** присланного — событие вне окна `occurredAt` (не старше
30 суток, не более чем на 5 минут в будущем) или с `metadata` больше 4 КБ
отбрасывается поштучно, остальные из пачки — нет; переотправлять разницу не
нужно. Запрос отклоняется целиком только на `VALIDATION_ERROR` (400),
`ANALYTICS_IDENTITY_REQUIRED` (400, нет ни токена, ни `deviceId`),
`RATE_LIMITED` (429) и 5xx.

Дедупликации на сервере нет (`jack5505/mahalla#267` открыт под неё) — повтор
пачки после таймаута задвоит события. `DefaultAnalyticsEventQueue`
(`app/src/main/java/uz/mahalla/data/network/analytics/DefaultAnalyticsEventQueue.kt`)
поэтому не ретраит пачку сразу после отказа, а оставляет её в очереди на диске
(Room, `analytics_events`, v4 → v5) до следующего вызова.

`deviceId` шлётся и под токеном — единственный способ сшить анонимную часть
воронки с авторизованной. Гео-заголовки ручке не нужны — она в белом списке
`GeolocationFilter`, по заявлению бэкенда.
