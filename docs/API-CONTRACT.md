...
**Форма `startTime`/`endTime` в `AppointmentResponse` не определена** (issue
#141). `/v3/api-docs` описывает `java.time.LocalTime` объектом
`{hour, minute, second, nano}` — так его рисует springdoc, — а Jackson с
`JavaTimeModule` отдаёт строку `"09:30:00"`; что именно придёт со стенда, под
токеном никто не проверял. Поэтому в DTO оба поля — `JsonElement?`, а разбирает
их `core/format/ServerLocalTime.kt` (`parseServerLocalTime`): он понимает обе
формы и строку без секунд. Жёсткий тип здесь роняет разбор **всей страницы**
(`ApiError.Serialization` на весь ответ), а не одного поля. Так уже было:
в «Моих активностях» из списка разом выпадали оба источника записей — и мастер,
и врач. Сверить форму — пунктом в пробу, когда появится
`CONTRACT_REFRESH_TOKEN`.
