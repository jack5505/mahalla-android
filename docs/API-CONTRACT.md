...
**`wallet/business` и `wallet/business/payouts` (issue #290, бэкенд
jack5505/mahalla#223) — единственные две ручки в этом файле, для которых
получилась не проба 401, а полная схема.** В этом прогоне анонимный
`/v3/api-docs` со стенда (`189-74-96-232.nip.io` из `BuildConfig.API_BASE_URL`)
внезапно отдал полное тело схемы без `CONTRACT_REFRESH_TOKEN` — так же не
проверялось ни разу раньше (`docs/changelog/`, «CONTRACT_REFRESH_TOKEN» в
`AGENTS.md`). Пользоваться этим как штатным способом сверки нельзя: доступность
анонимной схемы не гарантирована ни разу больше, оба ⚠️ у соседних ручек этого
файла (`PUT`/`DELETE food/items/{itemId}` в `BusinessApi`) как раз о том, что
`/v3/api-docs` то отдаёт схему анонимно, то требует токен.

- `GET wallet/business` → `ApiResponseWalletResponse` — **тот же `WalletResponse`**,
  что у `GET wallet`; отдельного DTO в клиенте нет, используется `WalletDto`.
- `POST wallet/business/payouts` → тело `PayoutCreateRequest{amount: int64 ≥ 1,
  cardNumber: string, \d{16}}`, ответ `ApiResponsePayoutResponse` →
  `PayoutResponse{id, amount, amountSom, cardNumber, status, createdAt}`.
  `amount` — тийины, как и везде в кошельке.
- **Отдельной ручки для списка заявок на вывод и их статусов нет.** В схеме —
  только эта одна, `POST`. Статус заявки на клиенте виден только из ответа
  самого запроса на создание (`BusinessEarningsViewModel.lastPayout`) — после
  перезапуска приложения или ухода с экрана он теряется. Если статус
  действительно меняется во времени (подтверждена/отклонена/выплачена), а не
  выставляется сразу и окончательно, — нужна либо `GET
  wallet/business/payouts`, либо `PayoutResponse` в `wallet/transactions`;
  ни то, ни другое схема сейчас не подтверждает. **NEEDS-PARTNER.**
- **Ни процента комиссии, ни минимальной суммы вывода в схеме нет вовсе** — ни
  в `WalletResponse`, ни в `PayoutResponse`, ни в `PayoutCreateRequest`
  (последний называет только технический минимум в одну тийину). Клиент их не
  показывает и не проверяет сверх этого технического минимума: сумма и номер
  карты, введённые человеком, проверяются сервером, а отказ показывается его
  текстом (`ApiFailure.userMessage`). Если эти числа всё же нужны на экране —
  тоже **NEEDS-PARTNER**, добавлять их в схему бэкенду, а не гадать на клиенте.
- **Комиссия и возврат в истории — обычные записи `wallet/transactions`, без
  своих полей.** `WalletTransactionResponse` не менялась (`type`, `direction`,
  `amount`, `referenceType`, `description`, `status` — те же, что и раньше);
  бизнес-версия истории использует ту же ручку, что и личный кошелёк
  (`BusinessRepository.earningsHistory` вызывает `WalletApi.transactions`
  напрямую, отдельного пути под бизнес-кошелёк в схеме нет). **Не проверено на
  живых данных**: эмулятора и второго тестового аккаунта в песочнице нет, а
  успешный вход требует SMS-кода. Если оба начисления по заказу и удержанная с
  него комиссия окажутся не двумя записями, а одной без разбивки, — экран
  покажет одну строку, а не выдуманную вторую.
