...
`app/src/main/java/uz/mahalla/data/network/pin/PinApi.kt` — issue #102. Пути и
поля сняты со схемы стенда (`/v3/api-docs`, 2026-09-04) — **коллизии springdoc
здесь нет**, все схемы уникальны, поля взяты как есть. Под токеном ни одна
ручка не проверялась: без него все три отвечают `401`, а без гео-заголовков —
`403 GEO_PERMISSION_REQUIRED`.

| Метод | Путь | |
|---|---|---|
| GET | `pin/status` | query `deviceId` (обяз.) → `{pinSet, biometricEnabled, lockedSecondsRemaining, pinChangedAt, lastUsedAt}` |
| PUT | `pin/change` | `{currentPin, newPin, deviceId}` — все обязательны, PIN `^[0-9]{6}$` |
| PUT | `pin/biometric` | `{enabled, deviceId, pin}` — **PIN обязателен** → `ApiResponseBoolean` |

Не подключены осознанно: `pin/set` и `pin/reset` (требуют свежей пары
`otpToken` + `otpCode` — установку PIN делает `auth/setup-pin` на входе),
`DELETE pin` (молча выключил бы app-lock), `POST pin/verify` (подтверждение
кодом делают сами `change` и `biometric`).

## SessionApi ⚠️

`app/src/main/java/uz/mahalla/data/network/auth/SessionApi.kt` — issue #102,
сняты со схемы стенда, под токеном не проверены.

| Метод | Путь | |
|---|---|---|
| POST | `auth/session/check` | `{device}` → `{sessionValid, pinRequired, user, reason}` |
| POST | `auth/pin-resume` | `{device, pin, lat, lng}` → токены (`AuthResponse`) |

**Оба требуют Bearer** — в отличие от анонимных `auth/pin-login` и
`auth/setup-pin`. Поэтому `PinApi` и `SessionApi` собираются на **основном**
