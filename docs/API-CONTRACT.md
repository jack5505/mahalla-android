...
**`PUT places/{id}`** — правка карточки места владельцем (issue #188), схема
снята со стенда 2026-09-10 (`/v3/api-docs`, `operationId: update`, тело —
`UpdateRequest`). Коллизии имени нет (issue #154 развёл `CreateRequest`/
`UpdateRequest`). Все поля необязательны — сервер не заявляет, что случится с
полем, которое не пришло в теле, а живым запросом это не проверить (`401` до
валидации, `CONTRACT_REFRESH_TOKEN` в CI нет). Клиент исходит из того, что это
частичное обновление, но подстраховывается: форма всегда открывается,
заполненная текущей карточкой, и уходит целиком — так поле, которое человек не
трогал, приезжает тем же значением, что было, независимо от трактовки бэкенда.

| Поле | Тип | Ограничение схемы |
|---|---|---|
| `name` | `string` | ≤ 300 |
| `description` | `string` | — |
| `address` | `string` | ≤ 500 |
...
| `phone` | `string` | ≤ 20 |
| `website` | `string` | ≤ 300 |
| `lat`, `lng` | `double` | -90..90 / -180..180 |

Ответ — та же `ApiResponsePlaceDetail`, что у `GET places/{id}`, но клиент её не
разбирает: карточка перечитывается отдельным запросом (тот же приём, что у
отправки отзыва) — один маршрут разбора DTO вместо двух.

**`POST reviews/{id}/reply`** — ответ владельца на отзыв (issue #188). Тело в
схеме объявлено безымянной картой (`additionalProperties: string`), имени поля
нет вовсе — тот же случай, что `walkin/accept`/`walkin/decline` в PR #161.
Ключ **выведен из ответа**: `ReviewResponse.ownerReply` — то же поле в ответе
`GET reviews/places/{placeId}`, уже подтверждённое (см. абзац про аватар выше).
Отправлен как `{"ownerReply": "..."}`. Не проверено живым запросом (`401` до
валидации, токена в CI нет) — если бэкенд ждёт другой ключ, тело уйдёт с полем,
которого он не узнает, и ответ пропадёт молча на его стороне, а не с ошибкой на
экране; при расхождении смотреть сюда в первую очередь.
