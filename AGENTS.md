...
- **Карточка — `OrderCard` из кита эпика 2 для всех пяти источников.**
  ...
  не делает.
- **Дата записи собирается из `apptDate` + `startTime`**, причём `startTime` —
  это **объект** `{hour, minute, second, nano}`, а не строка: Jackson так
  сериализует `java.time.LocalTime` без `JavaTimeModule`. Разворачивается
  местная дата в `Asia/Tashkent`, а не в UTC — иначе запись на 09:00
