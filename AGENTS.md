...
## Этап: две формы услуг — заказать и выставить (issue #71)

Issue просил «два форма: форма заказа и форма выставления услуги». Это две
стороны одной сделки, и обе уже поддержаны бэкендом — приложение о них просто
не знало.

**Контракт снят со стенда** (`https://189-74-96-232.nip.io/v3/api-docs` +
прямые curl'ы; дизайн-репо агенту по-прежнему недоступно):

| форма | эндпоинт | контракт | без токена |
|---|---|---|---|
| заказ услуги (клиент) | `POST walkin/send` | `{placeId, userName, serviceName}` → `Response` (статус, `queuePosition`, `estimatedWaitMinutes`, `counterTime`, `barberNote`) | `401` |
| анкета исполнителя | `GET`/`POST freelancers/me` | `ProfileResponse` (`name, profession, bio, city, phone, hourlyRate, experienceYears, isAvailable, ratingAvg, ratingCount`) | `401` |
| «принимаю заказы» | `PUT freelancers/me/toggle-availability` | без тела и без параметров, ответ — пустой конверт | `401` |
| каталог мастеров | `GET freelancers?profession&city&page&size` | страница `ProfileResponse` | **200** (анонимно) |

Весь код — в `feature/services/` (домен → данные → две формы), обе ручки на
**основном** Retrofit: `@RefreshClient` Bearer не ставит.

- **Домен** `feature/services/domain/`:
  - `ServiceOrderForm` + `ServiceOrderValidator` — поля ровно те, что принимает
    бэкенд: ни времени, ни комментария в контракте нет, а поле, которое никуда
    не уедет, на форме хуже его отсутствия. Название услуги бэкенд объявляет
    необязательным, **форма требует его**: заявка «Азиз, ???» мастеру
    бесполезна — он всё равно перезвонит спрашивать, и смысл заочной очереди
    теряется.
  - `ServiceRequest` + `ServiceRequestStatus` (10 статусов схемы + `Unknown`):
    `isFinal` — дальше состояние само не изменится, `isRejected` — только по
    нему рисуется «отправить новую заявку». Незнакомый статус читается как
    «заявка в работе», а не прячет её.
  - `ServiceOffer`, `ServiceOfferForm` (+ `of()` из ответа сервера),
    `ServiceOfferValidator`. Обязательны имя, профессия и город: по ним
    исполнителя ищут в каталоге (`?profession&city`), без них анкета не
    находится вовсе. Цена и стаж — строками в форме (поле умеет быть пустым и
    недописанным, `Long` не умеет), потолок цены `100 000 000` не эстетический:
    `hourlyRate` у бэкенда `int32`. Ноль в анкете значит «не указано» и
    возвращается в форму пустым полем — иначе анкета обещала бы бесплатную
    работу и нулевой стаж. Телефон проверяет `PhoneNumberValidator` онбординга
    (тот самый, что «переиспользуется и в профиле»).
- **Данные** `feature/services/data/`: `ServicesApi` + DTO, `ServicesRepository`
  (интерфейс и `Default`). Кэша нет намеренно — заявка живёт минуты, а анкету
  человек редактирует сам. Разбор мягкий: заявка **без `id`** — отказ
  (`ApiError.Serialization`), а не молчаливое «отправлено» на несуществующей
  заявке; `queuePosition: 0` — это «сервер её ещё не считал», а не «вы 0-й в
  очереди». Мапперы вызываются **внутри** `apiCall` — иначе выброшенное ими
  исключение улетело бы в ViewModel мимо `ApiResult`.
  **`404` на `GET freelancers/me` — не ошибка, а `Success(null)`**: «анкеты ещё
  нет» это первый вход в форму, и экран ошибки закрыл бы его навсегда. Тот же
  смысл принимается и кодом в конверте (`*NOT_FOUND`).
- **Форма заказа** (`ui/order/`): имя предзаполняется из аккаунта, но остаётся
  редактируемым (услугу заказывают и для другого человека), и подставляется
  только в нетронутое поле. После отправки экран показывает **не «готово», а
  состояние заявки** — статус, место в очереди, ожидание, встречное время и
  заметку мастера: «отправлено» это ещё не «мастер согласился».
- **Форма выставления** (`ui/offer/`): анкета читается у сервера при открытии
  (она общая для всех устройств), при первом входе имя и номер берутся из
  аккаунта. Непрочитанная анкета **блокирует форму** — сохранение поверх неё
  затёрло бы серверную пустыми полями. Переключатель «принимаю заказы» не
  передаёт желаемое значение (`toggle-availability` его не принимает), поэтому
  после успеха анкета **перечитывается** у сервера — как список устройств после
  доверия (issue #61). Форма после сохранения пересобирается из ответа: сервер
  мог что-то нормализовать, и показывать надо принятое им.
- **Точки входа**: карточка места → «Встать в очередь» (действие `Queue`) и
  профиль → «Мои услуги». Анкета своя у каждого, поэтому вход в неё из
  профиля, а не из каталога мастеров.
- **Попутно починено: кнопки вертикалей на карточке места не показывались
  вообще.** С issue #53 бэкенд не отдаёт `hasQueue`/`hasBooking`/`hasOrdering`,
  а из категории они не выводились — `PlaceCapabilities` всегда оставался
  пустым, и `PlaceActions.resolve` не добавлял ни одного действия вертикали
  (заказ еды в том числе). Теперь есть `PlaceCapabilities.forCategory`:
  `Master` → очередь. Остальные категории по-прежнему без кнопок **осознанно**
  — у еды пути `FoodApi` расходятся с бэкендом (`docs/UI-INVENTORY.md` §3.1), у
  игровых зон и кино экранов нет, а кнопка, ведущая в никуда, хуже её
  отсутствия. `PlaceDetailsEffect.OpenVertical` теперь везёт и `placeName`:
  читать состояние карточки из обработчика эффекта нельзя — он собран один раз
  и видел бы данные на момент подписки.

Проверено: `./gradlew testDebugUnitTest` — **922 теста в 100 классах, 0
падений**; `lintDebug` — `No issues found`; `assembleDebug` и `assembleRelease`
— BUILD SUCCESSFUL. Новые тесты: `ServiceOrderValidatorTest`,
`ServiceOfferValidatorTest`, `ServiceRequestStatusTest`, `ServicesRepositoryTest`
(MockWebServer: тела запросов, `+998…` в E.164, пустое поле не отправляется,
404 и `*NOT_FOUND` как «анкеты нет», 2xx с `success:false`, заявка без `id`),
`ServiceOrderViewModelTest` (Robolectric — `toRoute()`),
`ServiceOfferViewModelTest`, `PlaceCapabilitiesTest`; дополнены
`RoutesSerializationTest`, `GraphAssemblyTest`, `PlaceDetailsViewModelTest`;
фейк — `testutil/FakeServicesRepository`.

**Не сделано / риски:**

- **Тело `POST freelancers/me` угадано по `ProfileResponse`.** В схеме оно
  перекрыто коллизией springdoc (один `CreateRequest` на анкету исполнителя и
  на создание отзыва — та же беда, что в `docs/UI-INVENTORY.md` §3.2), а
  проверить запросом нечем: ручка анонимно отвечает `401`. Отправляются те же
  имена полей минус вычисляемые сервером. **Это первое, что надо проверить на
  живом аккаунте** — и повод попросить `springdoc.use-fqn=true` у
  `jack5505/mahalla`.
- **Успешный путь на живом стенде не проверен** (эмулятора в CI нет, вход
  требует SMS-кода): и заявка, и анкета проверены только на отказах и на
  MockWebServer. Каталог `GET freelancers` при этом отвечает `200` с пустым
  списком — исполнителей на стенде пока нет вовсе.
- **Заявка не перечитывается**: `GET walkin/{id}` в контракте нет (есть только
  дашборд мастера `GET walkin/barber/dashboard`), поэтому экран показывает то
  состояние, что приехало в ответе на отправку. Как мастер ответил, человек
  узнает, только отправив новую заявку. Опрос статуса, как у заказа еды, станет
  возможен, когда бэкенд отдаст чтение заявки — задача для `jack5505/mahalla`.
- **Каталога мастеров в приложении нет**: `GET freelancers` не подключён —
  анкету можно выставить, но найти чужую пока негде (это задача C7 из
  `docs/UI-INVENTORY.md`, она шире, чем «две формы»).
- **Заявка на услугу не попадает в таб «Заказы»** — тот по-прежнему заглушка
  (задача A3 инвентаризации).
- **`walkin/send` идёт к заведению, а не к конкретному мастеру**: `staffId` в
  ответе есть, в запросе — нет. Выбрать мастера внутри заведения нечем.
- Скриншот-тестов по-прежнему нет: обе формы, состояние заявки и переключатель
