...
## Этап: «Еда» под реальный контракт бэкенда (issue #63, задача T4)

Единственная полностью написанная вертикаль на живом сервере была мертва: из
шести вызовов `FoodApi` (эпик 5, написан по здравому смыслу — дизайн-репо тогда
не подключалось) не работал ни один.

**Контракт снят со стенда** (`https://189-74-96-232.nip.io/v3/api-docs` +
прямые curl'ы; дизайн-репо агенту по-прежнему недоступно) и **разошёлся с
анализом #59**: две ручки, которых там значились отсутствующими, на бэкенде уже
есть. Перепроверять контракт перед работой обязательно — он двигается.

| было в приложении | стало | проверено curl'ом |
|---|---|---|
| `GET places/{id}/menu` | `GET food/places/{placeId}/menu` → `data: [MenuResponse]` | 200 **без токена**; неизвестное заведение → `data: []`, а не 404 |
| `POST orders` | `POST food/orders` | 401 без токена |
| `GET orders/{id}` | `GET food/orders/{orderId}` — **появился** | 401 без токена |
| `POST orders/{id}/cancel` | `POST food/orders/{orderId}/cancel` — **появился** | 401 без токена |
| `POST places/{id}/promo` | `GET promotions/check?code&placeId&orderAmount` | 404 `NOT_FOUND` на выдуманный код |
| `GET wallet/balance` | `GET wallet` | закрыто раньше, в issue #62 |

Всё в конверте `{success, data, error{code,message}}` — как каталог после #53.

**Что решено про поля, которых в контракте нет.** Правило одно: не собирать с
человека то, что молча выбрасывается по дороге на сервер, и не показывать
сумму, которой не будет в счёте.

- **Модификаторы позиции** (`ItemResponse` их не знает, `PlaceOrderRequest` не
  принимает): маппер их не создаёт, поэтому `MenuItem.hasOptions` всегда
  `false` и шторка не открывается — позиция кладётся в корзину одним нажатием.
  Домен `MenuOptionRules`, `OptionGroup` и сама шторка оставлены под будущий
  контракт: они чистые и покрыты тестами, а выбрасывать их значит писать заново.
- **Время доставки и комментарий** — удалены из формы, из `CheckoutForm`, из
  валидатора и с экрана вместе с `DeliverySlots`. Слоты были самой аккуратной
  частью эпика 5, но отправить их некуда.
- **Промокод** — самый неприятный случай: `GET promotions/check` есть и
  работает, а поля `promoCode` в `PlaceOrderRequest` **нет**, то есть донести
  проверенный код до заказа нечем. Проверка и разбор ответа реализованы и
  покрыты тестами, а поле в корзине скрыто одной константой
  (`CartState.promoSupported`): скидка, показанная в корзине и не попавшая в
  счёт, — враньё про деньги. **Вопрос к `jack5505/mahalla`.**
- **Стоимость доставки и минимальная сумма заказа** в меню отсутствуют.
  Настоящую `deliveryAmount` считает бэкенд и присылает уже в ответе на заказ,
  поэтому итог на checkout'е при доставке заведомо меньше счёта — под кнопкой
  появилась строка «стоимость доставки посчитает заведение». Молчать здесь
  нельзя: это опять деньги.
- **Название заведения** не отдаёт ни `MenuResponse`, ни `OrderResponse`. Берём
  из кэша каталога (`PlaceDao.byId`): в меню приходят с карточки места, то есть
  запись там уже лежит. Нет записи — пустое имя, а не выдуманное.

Прочее по контракту:

- **Статусы заказа** — перечисление бэкенда: `NEW`, `ACCEPTED`, `PREPARING`,
  `READY`, `IN_DELIVERY`, `DELIVERED`, `CANCELLED`, плюс новый для приложения
  `REFUNDED` (финальный, своя подпись). Прежние `created`/`confirmed`
  разъезжались бы с сервером и делали каждый заказ «в работе» — то есть
  прятали бы кнопку отмены.
- **Способы** — `DELIVERY`/`PICKUP` и `WALLET`/`CASH` в верхнем регистре.
  `DINE_IN` бэкенда маппится в самовывоз: для оформления это то же самое
  (адрес не нужен), а третья кнопка объясняла бы разницу, которой в форме нет.
- **`totalAmount` важнее собственной арифметики**: `Order.totalSum` приходит от
  сервера и показывается на экране заказа, `totals` осталась ради разбивки.
- **`orderNumber`** («A-1042») показывается в шапке заказа вместо оценки
  готовности: `etaMinutes` бэкенд не отдаёт, а номер называют на выдаче.
- **Дата** — общий `parseServerInstant`: Jackson отдаёт `LocalDateTime` без
  зоны, и иначе дата пуста у всех (та же грабля, что в #53 и #61).
- Ошибки экранов еды переведены на `result.failure` (issue #34) — это был
  записанный здесь же незакрытый пункт эпика 5.

Проверено: `./gradlew testDebugUnitTest` — **863 теста в 93 классах, 0
падений**; `lintDebug` — 0 issue; `assembleDebug` и `assembleRelease` — BUILD
SUCCESSFUL. Тесты: переписанный `FoodRepositoriesTest` (MockWebServer: пути
`food/...`, конверт, `success:false`, пустое меню неизвестного заведения, тело
заказа ровно из пяти полей, имя заведения из кэша, дата без зоны, query
промокода и `valid:false`), `PromoCodeTest`, `CheckoutValidatorTest`,
`OrderStatusFlowTest` (сверка со значениями стенда), `CartCalculatorTest`,
`CartViewModelTest` (снятие скидки при изменении корзины), `CheckoutViewModelTest`.

**Не сделано / риски:**

- **На живом бэкенде не проверено ничего из заказов**: `food/orders` требует
  входа, а SMS-кода в CI нет. Анонимно проверены только меню и промокод.
  **Первое, что надо прогнать руками: создание заказа, чтение статуса и
  отмена.**
- **Каталог стенда пуст** (см. issue #53), поэтому меню реального заведения
  открыть не на чем: `food/places/{id}/menu` отвечает `data: []` на любой id.
  Это ответ сервера, а не поломка клиента.
- **Промокод недоступен пользователю** — см. выше. Вернуть его = добавить
  `promoCode` в `PlaceOrderRequest` на бэкенде и поменять `PROMO_SUPPORTED`.
- **Итог заказа с доставкой на checkout'е заведомо неполный** (сумма доставки
  известна только серверу). Правильное решение — отдавать её в меню или в
  отдельной ручке расчёта; сейчас закрыто предупреждающей строкой.
- **Модификаторы в UI недостижимы**, хотя код шторки жив. Если контракт их не
  получит в обозримое время, `OptionsSheet` и `MenuOptionRules` честнее
  удалить, чем держать.
- `GET food/orders/my` (страница заказов) не используется: списка «мои заказы»
  в приложении пока нет — это отдельный экран.
