...
- **Экран** `feature/onboarding/ui/BackendUrl{Screen,ViewModel,Contract}.kt` —
  ...
  и позже — кнопка «Изменить адрес сервера» на welcome: опечатка в хосте иначе
  запирала бы приложение навсегда (пройти онбординг без сети нельзя, а экран
  адреса больше не показывался бы).
...
- **Тесты (+31, всего 491 в 58 классах, 0 падений)**: `BackendUrlTest`
  (нормализация и rewrite, включая экранирование в сегментах и чужой путь),
  `BackendUrlStoreTest`, `BackendUrlInterceptorTest` (Retrofit собран на
  `baseUrl` сборки, запрос приезжает на MockWebServer с правильным путём и
  query), `BackendReachabilityTest`, `BackendUrlViewModelTest`,
  плюс новые случаи в `RootViewModelTest` (стартовый экран и его фиксация).

**Не сделано / риски:**

- **Адрес проверялся только на MockWebServer** — на устройстве (эмулятора в CI
  нет) не пробовали ни ввод, ни тайлы `http` на API 26+: у release-сборки
  `usesCleartextTraffic` не включён, поэтому `http://192.168.…` на релизе
  работать не будет. Решить, нужен ли network-security-config для локальных
  адресов, — вопрос к продукту.
- Экран адреса ничем не защищён: любой пользователь release-сборки может
  увести приложение на свой сервер. Если это не задумано, экран стоит
  ограничить debug-сборкой или спрятать за длинным нажатием.
- Кнопка «Изменить адрес сервера» живёт на welcome; после входа (в профиле)
  сменить адрес пока нельзя.
- В `MahallaNavHost` «экран стартовый или нет» определяется по
  `previousBackStackEntry == null` — работает, но при появлении новых входов
  на этот маршрут условие лучше вынести в параметр.
