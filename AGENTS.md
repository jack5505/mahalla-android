...
- **Восемь часов — клиентский потолок брони** (`MAX_HOURS`): у бэкенда его
  ...
  настоящими деньгами.
- **Два входа**: кнопка «Забронировать» на карточке клуба
  (`PlaceCapabilities.of(Playground) → booking = true`) и строка «Мои брони» в
  ...
  `ordering` по-прежнему выключен («Заказать» — вне объёма #96 и #98), бронь у
  врачей и мастеров ведёт другой контроллер (`appointments`), экранов под него
  нет — кнопки там тоже нет.
- **Состояния разложены руками**, а не через `ScreenStateHost`: тот рисует
  `ApiErrorState` с собственной прокруткой, а внутри `LazyColumn` вложенная
