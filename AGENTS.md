...
### Слияние с вертикалью «Больницы» (issue #99)

Пока PR #119 ждал, в `main` влилась вертикаль «Больницы» (#117), и ветка
отстала: **GitHub не смог собрать merge-ref, поэтому CI на PR не запускался
вовсе** — у ветки не было ни одного прогона, а PR показывал «проверки не
пройдены». То есть «CI упал» здесь означало «CI не смог начаться».

Конфликты были только текстовые, в трёх файлах, и разрешены в пользу обеих
сторон: `values/strings.xml` и `values-ru/strings.xml` (оба эпика дописывали
строки в конец) и импорты в `GraphAssemblyTest`. Смысловых пересечений нет:
`ProfileScreen` слился сам (у него от «Больниц» появился параметр
`onOpenMyDoctorAppointments`, от этой задачи — блок аватара), `ProfileViewModel`
тоже. После слияния: `testDebugUnitTest` — **1350 тестов в 135 классах, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
`assembleRelease` — BUILD SUCCESSFUL.
