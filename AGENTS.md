...
4. Блокер 4.2 закрыт (Yandex MapKit, эпик 4.2), но `MapScreen` всё ещё рисует
   `MapCanvasPlaceholder` — подключить `MapCanvas` к экрану и решить судьбу
   сеточного `MarkerClusterer`.
