...
## Инвентаризация: в каком состоянии проект сейчас (снимок 2026-08-28, issue #36)

**Читай этот раздел первым.** Всё, что ниже заголовка «История по этапам», —
хроника: зачем каждое решение принято и на какие грабли уже наступали. Она
полезна, когда правишь конкретную подсистему, но чтобы понять «где мы вообще»,
хватает этого раздела.

### Коротко

Готов **фундамент и первый пользовательский путь**: приложение запускается,
спрашивает адрес сервера, проводит через онбординг с входом по SMS-коду и
PIN'у, показывает главную с поиском мест, карточку места и карту (движок
подключён, но экран карты всё ещё рисует заглушку). **Продуктовых вертикалей
нет ни одной**: очередь, бронь, заказ, оплата, кошелёк, бизнес-панель — пустые
экраны или их нет вовсе. По ТЗ 35 экранов; в графе навигации сейчас **14**, из
них два (`Orders`, `Wallet`) — заглушки-скелетоны.

Что сделано, крупными мазками: каркас Gradle → тема → UI-кит → архитектурный
фундамент (DI/сеть/БД/навигация) → онбординг и авторизация → discovery (главная,
поиск, карточка места, карта) → инструменты разработчика (ввод адреса бэкенда,
Chucker, доверие самоподписанному сертификату, показ ошибок бэкенда).

### Карта кода (`app/src/main/java/uz/mahalla/`)

| Пакет | Что там | Состояние |
| --- | --- | --- |
| `ui/theme/` | `Color/Type/Shape/Theme` варианта B «Focus», `Spacing`, Inter | готово |
| `core/ui/components/` | UI-кит: кнопки, поля, чипы, тоглы, карточки, бары, шторки, состояния экрана, снекбар, `ErrorDetails` | готово |
| `core/{result,format,locale,di}` | `ApiResult`/`ApiError`/`ApiFailure`/`ServerErrorParser`, форматтеры денег/дат/талонов, per-app languages | готово |
| `data/network/` | OkHttp+Retrofit, `AuthInterceptor`, `TokenAuthenticator`, `BackendUrl*` (адрес из приложения), `tls/` (пин по отпечатку), `inspector/` (Chucker) | готово |
| `data/db/` | Room `MahallaDatabase` **version = 2**, `exportSchema = false`; сущности `PlaceEntity`, `OrderEntity`, `CartDraftItemEntity` | places используется, orders/cart — заготовки под вертикали |
| `data/prefs/` | DataStore: `SettingsDataStore` (язык, тема, город, биометрия, адрес, пин сертификата), `SessionStore` (токены) | готово |
| `data/security/` | PIN: PBKDF2 + AndroidKeyStore, `BiometricAvailability` | готово |
| `navigation/` | typed routes, `MahallaNavHost` (14 назначений), bottom nav, deep link `mahalla://place/{placeId}` | готово для текущих экранов |
| `feature/onboarding/` | welcome, phone, otp, pin, biometric, geo + экран адреса бэкенда | готово |
| `feature/auth/` | `AuthRepository`, `OtpChallenge`, `OtpFailure` | готово, **контракт с бэкендом не сверен** |
| `feature/discovery/` | домен фильтров/сортировки/истории, `CatalogRepository` (сеть + фоллбэк на Room), главная и поиск | готово |
| `feature/place/` | `PlaceDetails`, часы работы, действия, экран карточки | готово |
| `feature/map/` | `MapKitInitializer`, `UserLocationProvider`, `canvas/` (Yandex MapKit), `MarkerClusterer`, экран | **полотно к экрану не подключено** |
| `feature/profile/` | язык, тема, адрес сервера, вход в Chucker | минимальный, продуктового профиля нет |
| `feature/orders/`, `feature/wallet/` | `ScreenSkeleton` на 16 и 32 строки | **заглушки** |

Цифры: **157** файлов исходников, **78** тестовых (67 классов тестов + 11 фейков
и хелперов в `testutil/`), **208** строк в `values/` (206 в `values-ru/`, два
`translatable="false"`). Instrumentation-тестов нет — каталога
`app/src/androidTest` в проекте вообще не существует.

Прогон на этой ветке 2026-08-28: `./gradlew testDebugUnitTest` — **566 тестов в
67 классах, 0 падений, 0 ошибок**; `./gradlew assembleDebug` — BUILD SUCCESSFUL
(в APK приезжает `libmaps-mobile.so`). Предупреждения компилятора: три штуки,
все про deprecated API (`Icons.Outlined.Send` и `Icons.Outlined.ReceiptLong` →
AutoMirrored-варианты, `fallbackToDestructiveMigration()` без параметра).

### Экраны: что реально работает

Написаны и покрыты тестами (но **на устройстве не проверен ни один** — эмулятора
в CI нет): **адрес бэкенда**, **welcome**, **телефон**, **код из SMS**, **PIN**,
**биометрия**, **геолокация/выбор города**, **главная (discovery)**, **поиск с
фильтрами**, **карточка места**, **профиль** (язык/тема/сервер/Chucker).

Работает наполовину: **карта** — весь код, не зависящий от SDK (загрузка,
кластеризация, выбор маркера, «моё местоположение»), готов и покрыт тестами;
Yandex MapKit подключён и полотно `MapCanvas`/`YandexMapCanvas` написано, но
`MapScreen.kt:94` по-прежнему вызывает `MapCanvasPlaceholder`.

Заглушки: **заказы**, **кошелёк** (демонстрируют только `ScreenSkeleton` и
формат сумм).

Нет вообще: профиль пользователя и его редактирование, очередь и талоны, брони,
меню заведения и корзина, оформление и статус заказа, платежи и пополнение
кошелька, подписки, написание отзыва (читаются — не пишутся), уведомления, чат
с поддержкой, бизнес-панель, app-lock на старте (флаги PIN и биометрии
сохраняются, но при запуске приложение их не спрашивает). Из карточки места
уже уходит эффект `OpenVertical` — вести ему пока некуда.

### Инфраструктура, о которой стоит знать до первой правки

- **Адрес бэкенда вводится в приложении** (issue #26). `BuildConfig.API_BASE_URL`
  — только значение по умолчанию; фактический адрес переписывает
  `BackendUrlInterceptor` на обоих клиентах. Право менять адрес — гейт
  `BuildConfig.BACKEND_URL_OVERRIDE` (debug — всегда, release — только по явной
  сборке). Тем же гейтом закрыты доверие самоподписанному сертификату и показ
  сырого тела ошибки.
- **Два OkHttp-клиента**: основной и `@RefreshClient` (без `TokenAuthenticator`)
  — вход, OTP и refresh ходят по второму, иначе 401 зациклился бы.
- **Ошибки бэкенда показываются пользователю** (issue #34): `ApiFailure` держит
  разобранный ответ сервера, `ApiResult.Failure` сравнивать по значению нельзя —
  сравнивай `result.error`.
- **Ключ MapKit** берётся из `MAPKIT_API_KEY` (env) или `local.properties`
  (`mapkit.apiKey=…`). Пустой ключ сборку не ломает — карта показывает
  объяснение.
- **Тема**: Dynamic Color выключен, язык по умолчанию **uz** (`values/`),
  ru — `values-ru/`; `StringResourceParityTest` не даст добавить строку в одну
  локаль и забыть вторую.

### CI: четыре workflow (в истории ниже сказано «три» — устарело)

| Файл | Когда | Что делает |
| --- | --- | --- |
| `ci.yml` | только вручную | `testDebugUnitTest` + `assembleDebug`; автозапуска на PR **нет** |
| `claude.yml` | `@claude` в issue/PR/ревью | агент в ветке `claude/*` |
| `claude-dev.yml` | вручную | агент + docker-бэкенд, postgres, redis; ветки `claude-dev/*` |
| `release-internal.yml` | вручную или тег `v*.*.*` | тесты → debug-APK → GitHub Release (prerelease) для внутреннего теста |

**Ограничения агента в `claude.yml`**: в `--allowedTools` только
`Bash(./gradlew:*)`. Значит `gh`, `git clone`, `git merge` агенту недоступны —
сверить ТЗ с дизайн-репозиторием и посмотреть открытые issue он не может, пока
пользователь не расширит список. Дизайн-репозиторий приезжает в `design-repo/`
только если задан секрет `DESIGN_REPO_PAT`; на 2026-08-28 он **не задан**.

### Технический долг: открытые замечания ревью (проверено — всё ещё в коде)

Ни одно не блокирует работу, но каждое стоит завести задачей:

- `TokenAuthenticator` — `synchronized` + четыре `runBlocking` (нужны `Mutex` и
  кэш результата refresh); два независимых клиента с раздельными пулами.
- `MahallaDatabase.exportSchema = false` — включить до первого релиза, иначе
  миграций не будет.
- Лимит попыток PIN живёт только в памяти; app-lock на старте не сделан;
  `refresh()` при 401 чистит сессию, но не PIN.
- Пин сертификата хранится без привязки к хосту (нужна пара `host + fingerprint`).
- `BottomNavItem.route: Any`; `City.fromId`/`City.Default` объявлены и не
  используются; `SearchEvent.QueryCleared` обрабатывается, но UI его не шлёт.
- `<plurals>` нет ни одного, хотя просятся: `otp_input_description`,
  `pin_input_description`, `map_markers_count`, `map_cluster_places`.
- Кит: backwards write `TextFieldValue` в `MahallaPhoneField`; `state.canSubmit`
  считается, но кнопка всегда `enabled`; `buttonColors` без
  `disabledContainerColor`; двойной отступ в `Sheets.kt`;
  `rememberInfiniteTransition` в каждом `SkeletonBox`.
- `StringResourceParityTest` не ловит непозиционные `%s`/`%d`;
  `TouchTargetTest`/`ComponentStateTest` наполовину сверяют константы сами с
  собой; `GraphAssemblyTest` не закрывает созданные клиенты и DataStore.
- `OpeningHoursCalculator.isOpenAt` говорит «неизвестно» там, где `weekSchedule`
  рисует «выходной»; пустой бейдж рейтинга в отзыве; `CategoryTile` использует
  `Modifier.clickable` на `Surface`.
- Три предупреждения о deprecated API в сборке (см. цифры выше). Держать их на
  нуле полезно: настоящие предупреждения иначе тонут в шуме — на этом уже
  обжигались (`ExperimentalSerializationApi` в `RoutesSerializationTest`).

### Чего нет в проекте как класса

- **Загрузчика изображений** (Coil/Glide) — вместо фото везде скелетоны.
- **Скриншот- и Compose-тестов**: `compose-ui-test` не подключён, эмулятора в
  CI нет. Всё визуальное проверяется глазами по `@ThemeLanguagePreviews`.
- **Брендовой иконки** — лаунчер и splash до сих пор на временной заглушке
  (пункт 1.6, ждёт логотип).
- **Обфускации**: `isMinifyEnabled = false` в release.
- **Аналитики и крэш-репортинга** (Firebase/AppMetrica) — не выбраны.
- **Проверки на живом устройстве**: ни одна фича не проверена руками —
  биометрия, карта, cleartext, Chucker, TLS-пин, копирование в буфер.

### Что делать дальше (порядок предложенный, не догма)

1. **Подключить `MapCanvas` к `MapScreen`** — единственный оставшийся шаг
   эпика 4.2, код полотна уже написан и покрыт тестами. Заодно решить судьбу
   `MarkerClusterer` (при родной кластеризации MapKit он не нужен на карте).
2. **Сверить контракт с бэкендом**: эндпоинты OTP (`auth/otp/*`) и каталога
   (`places*`) написаны по здравому смыслу, а не по реальному API. Для этого
   пользователю нужно расширить `--allowedTools` (`gh`, `git clone`) или задать
   `DESIGN_REPO_PAT`.
3. **Первая вертикаль** — из карточки места уже уходит эффект `OpenVertical`,
   которому некуда вести. По ТЗ ближайшая: food (меню → корзина → оформление →
   статус заказа); сущности `OrderEntity` и `CartDraftItemEntity` в Room уже
   лежат под это.
4. **Загрузчик изображений** — без него ни одна витрина не выглядит готовой.
5. **App-lock на старте** (PIN/биометрия при запуске) — флаги сохраняются, но
   не используются; заодно перенести лимит попыток PIN в DataStore.
6. **Заказы и кошелёк** вместо заглушек.
7. Перед релизом: `exportSchema = true` + миграции, брендовая иконка,
   `isMinifyEnabled`, проверка лицензии Yandex MapKit.

**Всегда**: `./gradlew testDebugUnitTest` и `./gradlew assembleDebug` до пуша.
Три эпика подряд `main` оставался некомпилируемым (`Color.kt`, затем `Type.kt` +
`MahallaApp.kt`) именно потому, что сборка откладывалась до PR.

---

# История по этапам

Дальше — хроника: что делалось в каждом эпике, какие решения приняты и на какие
грабли уже наступали. Отдельные утверждения здесь устарели (их поправляет
инвентаризация выше) — это нормально, разделы намеренно не переписываются
задним числом.
...
- CI/Cloud-разработка (решение пользователя): `.github/workflows/` — тогда три
  workflow (сейчас четыре, `release-internal.yml` добавлен позже — см.
  инвентаризацию), все на официальном `anthropics/claude-code-action@v1` с авторизацией
...
# Окружение и правила (актуально всегда)
...
## Настройка окружения пользователем (историческая справка)

Актуальный список задач — в разделе «Инвентаризация» в начале файла, пункт
«Что делать дальше». Здесь остались только разовые действия по настройке
репозитория; часть из них уже выполнена.
...
Локальный прогон на машине разработчика: SDK в `.sdk/`, cmdline-tools,
platform-tools, `platforms;android-35`, `build-tools;35.0.0`, лицензии приняты
(`yes | sdkmanager --licenses`); дальше `./gradlew assembleDebug` и
`./gradlew testDebugUnitTest`.

Что пользователю стоит донастроить (агент это сделать не может — прав на
`.github/workflows` у GitHub App нет):

- **Расширить `--allowedTools` в `claude.yml`**: сейчас там только
  `Bash(./gradlew:*)`, поэтому агент не может ни склонировать дизайн-репозиторий,
  ни посмотреть issue через `gh`. Из-за этого вёрстка до сих пор не сверена с
  `TZ-ANDROID.md` и `SCREENS.md`.
- **Секрет `DESIGN_REPO_PAT`** — иначе `design-repo/` не появляется и агент
  работает без ТЗ и макетов (на 2026-08-28 не задан).
- **Секрет `MAPKIT_API_KEY`** и проброс его в env шага сборки в `ci.yml` /
  `claude*.yml` — без ключа карта показывает объяснение вместо тайлов.
- **Переменная `BACKEND_IMAGE`** (+ `BACKEND_PORT`, `BACKEND_HEALTH_PATH`) —
  без неё `claude-dev.yml` поднимается без бэкенда.
...
У файла две части, и правятся они по-разному.

1. **Инвентаризация в начале** — единственное место, где написано «как сейчас».
   Обновляй её **каждый раз**, когда меняешь состав экранов, слоёв, workflow'ов
   или закрываешь пункт техдолга: следующий агент читает её первой и по ней
   решает, за что браться. Устаревшая строка здесь хуже, чем её отсутствие.
2. **История по этапам** — дописывается новым разделом в конец, задним числом
   не переписывается. Её задача — объяснить, почему решение принято именно так
   и на какие грабли уже наступали.

Практически: закончил этап — добавь раздел в историю и **синхронизируй
таблицы и списки инвентаризации** (карта кода, экраны, техдолг, «что делать
дальше»), а в отчёте укажи результат `./gradlew testDebugUnitTest`.
