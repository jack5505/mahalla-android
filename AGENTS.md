...
## Этап: картинки — Coil и `MahallaAsyncImage` (issue #60, ветка claude/issue-60-*)

Загрузчика изображений в проекте не было вовсе: `logoUrl`/`coverUrl`/`photoUrl`
приезжали с бэкенда и использовались только как `.size` в галерее-скелетоне.
Из-за этого приложение выглядело пустым сильнее, чем от любой недоделанной
вертикали (инвентаризация UI, задача **A1** в `docs/UI-INVENTORY.md`).
...
- **Библиотека**: `io.coil-kt:coil-compose` 2.7.0. Не третья ветка: она тянет
  ...
  Android поверх уже настроенного OkHttp.
- **`core/image/MahallaImageLoader`** — сборка `ImageLoader` вне DI-модуля (по
  той же причине, что и `NetworkFactory`: конфигурацию проверяет тест, а не её
  копия). Crossfade 200 мс, память 20%, диск 64 МБ в `cacheDir/image_cache`,
  `respectCacheHeaders(false)` — бэкенд отдаёт медиа без заголовков
  кэширования, иначе дисковый кэш не работал бы вовсе.
- **Клиент для картинок** — `imageClient(client)`: тот же пул соединений и то
  же доверие сертификату стенда (issue #32), но с очищенным списком
  интерцепторов и `Authenticator.NONE`. Снято ровно три вещи и каждая по делу:
  `Authorization` (ссылка ведёт куда угодно, Bearer не должен уезжать на чужой
  хост), подстановка адреса и гео-заголовки (ссылка уже абсолютная), инспектор
  трафика (лента из сотни картинок вытеснила бы из Chucker'а настоящие
  запросы). Клиент берётся через `dagger.Lazy`, а `MahallaApplication`
  реализует `ImageLoaderFactory` с `Provider<ImageLoader>` — сетевой стек не
  поднимается на старте ради ещё не запрошенной картинки.
- **Относительные ссылки**: бэкенд отдаёт то `https://…`, то `/media/…`, то
  `media/…`, а адрес API задаёт пользователь (issue #26) — дописать хост
  сборкой нельзя. `ImageUrl.resolve(base, raw)` + Coil-интерцептор
  `BackendImageUrlInterceptor` подставляют **текущий** адрес в момент загрузки:
  после переезда стенда картинки едут туда же, куда и запросы. Схемы ограничены
  `http`/`https`/`data` — строку присылает сервер, и `file://`/`content://`
  показали бы в списке мест кусок локального хранилища.
- **Кит** (`core/ui/components/AsyncImages.kt`): `MahallaAsyncImage` (скелетон →
  фото с проявлением → фоллбэк-иконка; своей высоты нет — размер задаёт
  вызывающий), `MahallaThumbnail` (64dp, списки), `MahallaAvatar` (круг, вместо
  фото — первая буква имени: у списка отзывов она различает авторов, а силуэт
  делает их одинаковыми). `SkeletonBox` принял `height: Dp?` — заглушка под
  фото занимает столько, сколько дал родитель.
- **Куда подключено**: `PlaceCard` (главная, поиск, карта — одна карточка на
  всех), галерея карточки места (`LazyRow`, 240×160dp, край следующей
  фотографии виден — значит листается), строка меню (`MenuItemRow`, фоллбэк —
  иконка блюда), аватар автора отзыва. Картинки рядом с названием декоративны
  (`contentDescription = null`): TalkBack не должен читать имя дважды, а у
  галереи подпись одна на весь блок.
- **Отзывы**: у `Review` появился `avatarUrl`, у `ReviewDto` — три вероятных
  имени поля (`userAvatarUrl`/`avatarUrl`/`userAvatar`) по той же причине, что
  и у автора с текстом: схема стенда перекрыта коллизией `Response` (issue #53).

Заодно вернул `lintDebug` в зелёное: на `main` он падал двумя ошибками
`PluralsCandidate`, приехавшими с эпиком 5 (`order_eta`,
`checkout_error_time_too_soon` — число рядом с существительным). Обе строки
переведены в `plurals` (uz one/other, ru one/few/many/other), вызовы — на
`pluralStringResource`.

Проверено: `./gradlew testDebugUnitTest` — **837 тестов в 90 классах, 0
падений, 0 ошибок**; `lintDebug` — `No issues found`; `assembleDebug` и
...
локальным схемам, битый URL, адрес без завершающего слэша),
`MahallaImageLoaderTest` (клиент без интерцепторов и с общим пулом,
интерцептор в `ImageLoader`, подстановка адреса, абсолютная ссылка и
не-строка проходят нетронутыми).

**Не сделано / риски:**

- **На устройстве не проверено** (эмулятора в CI нет, каталог стенда пуст —
  `places/nearby` и `search` отвечают `200` с `data: []`): ни одна настоящая
  картинка в этом прогоне не загружалась. Проверять после наполнения каталога —
  в первую очередь относительные ссылки и http-медиа в release
  (`network_security_config` его запретит, и это правильно).
- **Загрузки своих файлов нет**: `POST media/upload` (multipart,
  `entityType`/`entityId`) и `GET media/entity/{entityId}` из задачи A1 не
  трогали — аватар профиля и фото отзыва вместе с экраном профиля это A4.
- **Профиль без аватара**: `avatarUrl` приходит в `user` из
  `verify-otp`/`pin-login`, но экран профиля пользовательских данных пока не
  показывает вовсе (задача A4) — подключать там нечего.
- **Плейсхолдеров-«битая картинка» с повтором нет**: не загрузилось — иконка,
  ручного retry на картинку не предусмотрено (Coil перезапросит при следующем
  показе элемента).
- Скриншот-тестов по-прежнему нет: фоллбэки и скелетоны проверялись глазами по
  `@ThemeLanguagePreviews` (превью рисуется без сети — видно ровно фоллбэки).

### Слияние с `main` и правки после ревью PR #66

Пока PR ждал, в `main` влилось шесть мержей (профиль и выход #61, отзывы #76,
обновление приложения #80, уведомления #81, анкеты #84, точка на карте #90,
Sentry #74, вертикаль «Еда» на реальном контракте #9). Двенадцать конфликтов,
из них два содержательных:

- **Блок «время доставки» и ETA заказа в `main` удалены** (issue #9 — реальный
  контракт бэкенда: ни `scheduledAt`, ни `etaMinutes` он не принимает). Значит
  и `plurals` `checkout_error_time_too_soon`/`order_eta`, которые этот PR завёл
  ради `PluralsCandidate`, вместе со строкой `checkout_error_time_required`
  уходят целиком — чинить лишними ресурсами лечение уже вылеченного не надо.
- **`MahallaApplication` совмещает оба исключения**: `CrashReporter` (issue #74,
  ставится в `onCreate` до всего) и `ImageLoaderFactory` (issue #60, ленивый
  `Provider`). Порядок важен: отчёты о падениях обязаны встать раньше кода,
  который падает, а загрузчик картинок — позже первого запроса картинки.
- Остальное — добавления с обеих сторон (`avatarUrl` рядом с `authorId` у
  отзыва, `parseServerInstant` из `main`, строки `image_*` рядом с анкетами,
  Coil рядом с Sentry в каталоге версий) и одна замена: имя автора отзыва в
  карточке теперь `if (isMine) «мой отзыв» else author`, где `author` — та же
  строка, что уходит в `MahallaAvatar`.

Закрыты оба блокирующих замечания авто-ревью:

- **Галерея больше не падает на дублях**: `LazyRow` ключевался самой ссылкой
  (`items(key = { it })`), а бэкенд ни уникальности, ни непустоты `photos` не
  обещает — второй одинаковый URL давал `IllegalArgumentException: Key … was
  already used` и уносил весь экран карточки. Теперь список чистится
  `filter(String::isNotBlank).distinct()` в `remember` — то же решение, что у
  `SearchHistory.decode` (PR #23).
- **Белый список схем `ImageUrl` перестал быть декорацией**: интерцептор на
  `resolve() == null` пропускал исходный запрос дальше, а `file://`,
  `content://` и `android.resource://` Coil грузит штатными фетчерами
  (`components { add(…) }` только дополняет дефолты). То есть подменённый
  бэкенд — адрес сервера в debug вводит пользователь (issue #26) — показал бы
  вместо фото заведения кусок локального хранилища, ровно то, от чего список и
  заводился. Теперь отвергнутая ссылка кончается `ErrorResult`
  (`UnsupportedImageUrl`), в загрузку не уходит вовсе, а пользователь видит
  тот же фоллбэк. Тест проверяет это через интерцептор, а не только через
  `resolve`: дыра была именно между ними.

Проверено после слияния: `./gradlew testDebugUnitTest` — **1104 теста в 117
классах, 0 падений, 0 ошибок**; `lintDebug` — `No issues found`;
`assembleDebug` и `assembleRelease` — BUILD SUCCESSFUL.

Осталось незакрытым замечание-не-блокер: `imageClient()` чистит только
`interceptors()`, но не `networkInterceptors()`. Сейчас Chucker и логгер
висят application-интерцепторами, так что клиент картинок чист; если кто-то
добавит network-интерцептор с секретами, чистить придётся оба списка.
